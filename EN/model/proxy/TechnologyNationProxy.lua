...
function slot0.nationPointFilter(slot0)
	...
	for slot4, slot5 in ipairs(slot0.groupListInCount) do
		if not slot5.maxLV or slot5.maxLV < TechnologyConst.MAX_LV then
			slot0.nationToPoint[slot5:getNation()] = slot0.nationToPoint[slot5.getNation()] + pg.fleet_tech_ship_template[slot5.id].pt_get
		else
		...
		for slot13, slot14 in ipairs(slot7) do
			if not slot0.typeBuffList[slot14] then
				slot0.typeBuffList[slot14] = {
					{
						pg.fleet_tech_ship_template[slot5.id].add_get_attr,
						pg.fleet_tech_ship_template[slot5.id].add_get_value
					}
				}
