...
function slot13(slot0)
	uv0((type(slot0) ~= "string" or string.match(slot0, "^[%w_.%-]+")) and nil, "cannot derive module name, use -n name")
...
typedef struct {
  ...
  uint8_t space[4096];
	...
	for slot17, slot18 in ipairs({
		...
		slot10.sect[slot17].name = slot7(1)

		slot3.copy(slot10.space + 1, slot18)
...
typedef struct {
  ...
  uint8_t space[4096];
	...
	if slot5 then
		slot11.fat.magic = uv2.bswap(3405691582.0)
		slot11.fat.nfat_arch = uv2.bswap(#({
			x86 = {
				3
			},
			x64 = {
	...
	for slot18 = 0, #slot14 - 1, 1 do
		if slot5 then
			...
			slot11.fat_arch[slot18].offset = slot10(0)
