...
function pg.Packer.GetProtocolWithName(slot0, slot1)
	...
	if nil then
		slot0._protocols[slot1] = uv0.Protocol.New(slot2, slot1, package.loaded["p" .. string.sub(slot2, 1, 2) .. "_pb"])

		return uv0.Protocol.New(slot2, slot1, package.loaded["p" .. string.sub(slot2, 1, 2) .. "_pb"])
	else
		return nil
	end
end
