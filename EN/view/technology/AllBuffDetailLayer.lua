...
function slot0.updateDetail(slot0)
	...
	slot1:make(function (slot0, slot1, slot2)
		if slot0 == UIItemList.EventUpdate then
			...
			setImageSprite(slot3, GetSpriteFromAtlas("ShipType", "buffitem_tec_" .. uv0.typeOrder[slot1 + 1]), true)
			Canvas.ForceUpdateCanvases()
			uv0:updateBuffList(uv0:findTF("Container", slot2), uv0.typeOrder[slot1 + 1])
		end
	end)
...
function slot0.updateBuffList(slot0, slot1, slot2)
	...
	slot3:make(function (slot0, slot1, slot2)
		if slot0 == UIItemList.EventUpdate then
			setText(slot3, AttributeType.Type2Name(pg.attribute_info_by_type[uv1[slot1 + 1]].name))
			setText(uv0:findTF("ValueText", slot2), "+" .. uv2[uv1[slot1 + 1]])
		end
	end)
