...
function slot3.createMapGrids(slot0)
	...
	for slot5 = slot0.start_x, slot0.end_x, 1 do
		slot6 = {
			[slot10] = cloneTplTo(findTF(slot1, "resources/gridtpl"), findTF(slot1, "bg/floorGrid"), slot5 .. "_" .. slot10)
		}
	...
	for slot4 = 1, 24, 1 do
		for slot8 = 1, 24, 1 do
			if slot0:hasGrid(slot4 - 1, slot8 - 1) then
				setText(slot0:getMapGrid(slot4 - 1, slot8 - 1).Find(slot10, "Text"), slot0.map.depths[slot0.map:GetIndex(slot4, slot8)])
			end
		end
	end
end
