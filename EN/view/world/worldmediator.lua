...
function slot0.handleNotification(slot0, slot1)
	...
	elseif slot2 == GAME.WORLD_TRIGGER_TASK_DONE then
		...
		if slot2 == GAME.WORLD_SUMBMIT_TASK_DONE then
		...
		if slot2 == GAME.WORLD_AUTO_SUMBMIT_TASK_DONE then
			if #slot3.task.config.task_ed > 0 then
				table.insert({}, function (slot0)
					pg.NewStoryMgr.GetInstance():Play(uv0.config.task_ed, slot0, true)
				end)
		...
		if slot2 == GAME.WORLD_ITEM_USE_DONE then
			slot6 = slot3.drops
			slot7 = {}
		...
		if slot2 == GAME.WORLD_RETREAT_FLEET then
