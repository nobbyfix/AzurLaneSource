...
function slot0.UpdateSign(slot0)
	setText(slot0.tabPanels[uv0.TabSign].Find(slot1, "reduce/text"), slot0.reduceSignDays)

	for slot6 = 0, slot0.tabPanels[uv0.TabSign].Find(slot1, "days").childCount - 1, 1 do
		slot7 = slot2:GetChild(slot6)
		slot9 = slot7:Find("checked")

		for slot15 = 0, slot7:Find("item").GetComponentsInChildren(slot8, typeof(Image)).Length - 1, 1 do
			slot7.Find("item").GetComponentsInChildren(slot8, typeof(Image))[slot15].color = slot6 + 1 <= slot0.reduceSignDays and Color.gray or Color.white
		end
...
function slot0.UpdateTask(slot0)
	...
	slot2 = UIItemList.New(slot0.tabPanels[uv0.TabTask].Find(slot1, "scrollview/viewport/list"), slot0.tabPanels[uv0.TabTask].Find(slot1, "scrollview/viewport/list/task"))
	...
	slot2:make(function (slot0, slot1, slot2)
		if slot0 == UIItemList.EventUpdate then
			...
			if slot13 == 0 then
				setSlider(slot4:Find("progress"), 0, slot3:getConfig("target_num"), slot3:getProgress())
				setText(slot4:Find("progress/text"), slot3.getProgress() .. "/" .. slot3.getConfig("target_num"))
...
function slot0.UpdatePt(slot0)
	for slot7 = slot0.tabPanels[uv0.TabPt].Find(slot2, "scrollview/viewport/list").childCount, #pg.return_pt_template.all - 1, 1 do
	...
	for slot8 = 0, slot3.childCount - 1, 1 do
		...
		for slot20 = 0, slot12:GetComponentsInChildren(typeof(Image)).Length - 1, 1 do
			slot16[slot20].color = slot8 + 1 <= slot0.battlePhase and Color.gray or Color.white
		end
		...
		if slot8 == slot0.battlePhase and slot9.pt_require <= slot0.bonusPoint then
			if slot0.ptAchieveTwId then
				LeanTween.cancel(slot0.ptAchieveTwId)
...
function slot0.ScrollPt(slot0, slot1, slot2, slot3)
	slot5 = slot0.tabPanels[uv0.TabPt].Find(slot4, "scrollview")
	...
	if slot2 then
		slot5:GetComponent(typeof(ScrollRect)).horizontalNormalizedPosition = math.clamp(math.max(slot1 * (slot9.preferredWidth + slot7:GetComponent(typeof(HorizontalLayoutGroup)).spacing) - slot6.rect.width * 0.5 + slot9.preferredWidth, 0) / (slot7.childCount * slot9.preferredWidth + (slot7.childCount - 1) * slot7.GetComponent(typeof(HorizontalLayoutGroup)).spacing - slot6.rect.width), 0, 1)
	else
