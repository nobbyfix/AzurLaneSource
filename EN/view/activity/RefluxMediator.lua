...
function slot0.register(slot0)
	...
	slot0.viewComponent:SetActivity(getProxy(ActivityProxy).getActivityByType(slot1, ActivityConst.ACTIVITY_TYPE_REFLUX))
	slot0.viewComponent:SetTasks(slot4)
	slot0.viewComponent:SetPlayer(slot6)

	if PlayerPrefs.GetInt(getProxy(PlayerProxy).getRawData(slot5).id .. "_" .. getProxy(ActivityProxy).getActivityByType(slot1, ActivityConst.ACTIVITY_TYPE_REFLUX).data2) == 1 then
