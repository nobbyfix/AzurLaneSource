...
function slot0.OnFirstFlush(slot0)
	...
	onButton(slot0, slot0.btn, function ()
		uv0:emit(ActivityMediator.GO_DODGEM, uv1:getConfig("config_client").stage_ids[math.min(uv0.progess + 1, uv0.maxday)])
	end, SFX_PANEL)
...
function slot0.OnUpdateFlush(slot0)
	...
	slot0.progess = math.min(slot0.activity.data2, uv0)
	slot0.maxday = math.min(slot2:DiffDay(slot0.activity.data1, slot2:GetServerTime()) + 1, uv0)
