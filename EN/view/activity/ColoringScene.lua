...
function slot0.initColoring(slot0)
	...
	triggerButton(slot0.paintsgroup[Mathf.Min(slot1, uv0.PageNums)], true)
...
function slot0.updatePage(slot0)
	...
	for slot6 = #slot0.paintsgroup, 1, -1 do
		if slot6 ~= slot0.selectedIndex then
			slot0.paintsgroup[slot6]:SetSiblingIndex(0)
...
function slot0.updateSelectedColoring(slot0)
	...
	for slot7 = 0, slot0.groupColor.childCount - 1, 1 do
		setText(slot0.groupColor:GetChild(slot7).Find(slot8, "icon/x/nums"), slot0.colorItems[slot0.colorGroups[slot0.selectedIndex].getConfig(slot1, "color_id_list")[slot7 + 1]] or 0)
	end
...
function slot0.updateCells(slot0)
	...
	slot4:AddPointClickFunc(function (slot0, slot1)
		...
		if uv2:getState() == ColorGroup.StateColoring then
			...
			if not uv2:canBeCustomised() then
				if uv1.selectedColorIndex == 0 or not uv2:getCell(slot3, math.floor(LuaHelper.ScreenToLocal(uv1.bg, slot1.position, GameObject.Find("UICamera"):GetComponent(typeof(Camera))).x / uv1.cellSize.x)) or uv2:hasFill(slot3, slot4) then
					return
				end
	...
	slot4:AddDragFunc(function (slot0, slot1)
		...
		if Application.isEditor or not uv1.zoom.enabled then
			uv1.bg.anchoredPosition.x = uv1.bg.anchoredPosition.x + slot1.delta.x * uv2.x
			uv1.bg.anchoredPosition.x = math.clamp(uv1.bg.anchoredPosition.x, -uv1.bg.rect.width * (uv1.bg.localScale.x - 1), 0)
			uv1.bg.anchoredPosition.y = uv1.bg.anchoredPosition.y + slot1.delta.y * uv2.y
			uv1.bg.anchoredPosition.y = math.clamp(uv1.bg.anchoredPosition.y, 0, uv1.bg.rect.height * (uv1.bg.localScale.y - 1))
			uv1.bg.anchoredPosition = uv1.bg.anchoredPosition
		end
	end)
...
function slot0.updateCell(slot0, slot1, slot2)
	...
	if slot0.colorGroups[slot0.selectedIndex].getState(slot3) == ColorGroup.StateFinish or slot6 == ColorGroup.StateAchieved then
		slot5 = slot0.colorGroups[slot0.selectedIndex].getCell(slot3, slot1, slot2)
	end
	...
	if slot4 or slot5 then
		...
		slot10 = slot8 or cloneTplTo(slot0.cell, slot0.cells, slot7):Find("text")

		if slot5 then
			setImageColor(slot8 or cloneTplTo(slot0.cell, slot0.cells, slot7):Find("image"), slot3.colors[slot5.type])
		else
