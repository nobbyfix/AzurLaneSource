...
function slot0.handleNotification(slot0, slot1)
	if slot1:getName() == GAME.ANSWER_START_DONE then
		slot0.viewComponent:startProcess(slot1:getBody().subject, slot1.getBody().answers)
	elseif slot2 == GAME.ANSWER_SUBMIT_DONE then
		if slot3.success then
			...
			slot4 = coroutine.create(function ()
				...
				if uv1.virgin and getProxy(ActivityProxy).getActivityByType(slot2, ActivityConst.ACTIVITY_TYPE_ANSWER):getConfig("config_client").plot[table.getCount(getProxy(AnswerProxy).scores) + 1] then
