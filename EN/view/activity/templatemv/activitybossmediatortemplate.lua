			...
			slot0:addSubLayers(Context.New({
				...
				data = {
					...
					OnConfirm = function (slot0)
						...
						slot1 = getProxy(SettingsProxy):isTipActBossExchangeTicket()

						if getProxy(PlayerProxy):getRawData():getResource(slot0.contextData.TicketID) > 0 and not slot0.contextData.stageTickets[slot1] then
							slot2 = 0
						...
						if not slot1 and slot2 then
							...
							slot4 = id2ItemId
							slot5 = slot0.contextData.TicketID or 1

							pg.MsgboxMgr.GetInstance():ShowMsgBox({
								noText = "text_inconsume",
								hideYes = true,
								content = i18n("tip_exchange_ticket", i18n(slot2[pg.item_data_statistics[id2ItemId(slot5)]])),
								custom = {
									{
										text = "text_consume",
										sound = SFX_CONFIRM,
										...
										iconName = {
											pg.item_data_statistics[id2ItemId(slot5)].icon
										}
									}
								},
								onNo = function ()
	...
	slot0:bind(slot0.ON_EX_PRECOMBAT, function (slot0, slot1, slot2)
		...
		seriesAsync({
			...
			function (slot0)
				...
				if not slot2 or not SYSTEM_BOSS_EXPERIMENT then
				...
				-- Decompilation error in this vicinity:
				--- BLOCK #1 14-32, warpins: 2 ---
				slot2 = slot3.contextData.exStageID

				slot0[slot1 + 10]:RemoveUnusedItems()
