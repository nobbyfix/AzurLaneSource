...
function slot0.updateBattleFleetView(slot0)
	local function slot1(slot0, slot1)
	...
	slot1(slot0._fleet:Find("vanguard"), slot0.challengeInfo.fleet:getShipsByTeam(TeamType.Vanguard, true))
end
	...
	local function slot1(slot0)
		for slot4, slot5 in ipairs(slot0) do
			if tf(slot5):Find("mouseChild") then
				uv0.eventTriggers[slot6:GetComponent("EventTriggerListener")] = true
...
function slot0.enabledTeamCharacter(slot0, slot1, slot2)
	for slot8, slot9 in ipairs(slot3) do
		slot0:enabledCharacter(slot9, slot2, slot0.challengeInfo.fleet[slot1][slot8], slot1)
	end
end
...
function slot0.displayFleetInfo(slot0)
	...
	uv0.tweenNumText(slot0._mainGS, _.reduce(slot0.challengeInfo.fleet.getShipsByTeam(slot1, TeamType.Main, false), 0, function (slot0, slot1)
...
function slot0.displayStrategyInfo(slot0, slot1)
	...
	function slot0.strategyPanel.onConfirm()
		slot1 = pg.strategy_data_template[uv1.id]

		if not uv0.challengeInfo.fleet:canUseStrategy(uv1) then
			return
		end

		uv0:emit(ChallengePreCombatMediator.ON_OP, {
			id = slot1.id,
			arg1 = uv1.id
		})
