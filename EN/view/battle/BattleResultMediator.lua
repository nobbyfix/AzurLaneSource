...
function slot0.register(slot0)
	...
	if slot0.contextData.system == SYSTEM_DUEL then
		...
		slot0.viewComponent:setRank(getProxy(PlayerProxy).getData(slot3), getProxy(MilitaryExerciseProxy):getSeasonInfo())
	...
	elseif slot9 == SYSTEM_WORLD then
		slot16 = getProxy(WorldProxy).GetWorld(slot11).GetActiveMap(slot12).GetFleet(slot13):GetTeamShipVOs(TeamType.Vanguard, true)
	...
	slot0:bind(uv0.ON_CHALLENGE_RANK, function (slot0)
		slot1 = getProxy(ContextProxy)

		if uv0 == SYSTEM_CHALLENGE then
			slot2 = getProxy(ChallengeProxy)

			if uv1.contextData.score <= 0 or slot2:getCurrentChallengeInfo():isClear() then
	...
	slot0:bind(uv0.ON_BACK_TO_LEVEL_SCENE, function (slot0)
		if uv0 == SYSTEM_ACT_BOSS then
			if getProxy(ContextProxy):getContextByMediator(ActivityBossBattleMediator) then
				slot2:removeChild(slot2:getContextByMediator(PreCombatMediator))
			end
		elseif uv0 == SYSTEM_ROUTINE then
			if slot1:getContextByMediator(DailyLevelMediator) then
				slot2:removeChild(slot2:getContextByMediator(PreCombatMediator))
			end
	...
	slot0:bind(uv0.ON_NEXT_CHALLENGE, function (slot0)
...
function slot0.showExtraChapterActSocre(slot0)
	if getProxy(ActivityProxy):getActivityByType(ActivityConst.ACTIVITY_TYPE_EXTRA_CHAPTER_RANK) and not slot2:isEnd() and getProxy(ChapterProxy):getActiveChapter() then
		...
		}):isActExtra() and slot2:getConfig("config_data")[1] == slot0.contextData.stageId then
			slot10, slot11 = ActivityLevelConst.getExtraChapterSocre(slot0.contextData.stageId, math.floor(slot0.contextData.statistics._totalTime), ActivityLevelConst.getShipsPower(slot0.contextData.prefabFleet or slot0.contextData.oldMainShips), slot2)
