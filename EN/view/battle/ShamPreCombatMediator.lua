...
function slot0.enter(slot0)
	...
	elseif not slot1.fleet:isValid() then
		...
		if slot1.active then
			slot0:save(function ()
				if uv0:getChapterCell(uv0.fleet.line.row, uv0.fleet.line.column) and slot1.attachment == ChapterConst.AttachRival and slot1.flag == 0 then
...
function slot0.openDockYard(slot0, slot1, slot2, slot3)
	...
	slot6 = coroutine.create(function ()
		...
		elseif uv0 == 1 then
			slot2 = 0
			slot3 = math.min(ChapterConst.ShamShipLimit - (#uv1:getShips() - #_(slot5):chain():filter(function (slot0)
		...
		elseif uv0 == 2 then
			for slot13, slot14 in pairs(slot6) do
				if not getProxy(PlayerProxy).playerAssists[slot13] or pg.TimeMgr.GetInstance():GetServerTime() - getProxy(PlayerProxy).playerAssists[slot13].timeStamp > 60 then
					table.insert({}, slot13)
				end
			end
	...
	return function (slot0, slot1, slot2)
		...
		if uv0 == 0 then
			...
			if not (uv1:getFriendShip() and slot3.id == slot0[1] and slot3 or uv1:getShip(slot0[1])) then
				if uv2 then
					if uv1.active and #uv3[uv2:getTeamType()] == 1 then
						pg.TipsMgr:GetInstance():ShowTips(i18n("ship_formationUI_removeError_onlyShip", uv2:getName(), "", Fleet.C_TEAM_NAME[uv2.getTeamType()]))
					else
		...
		elseif uv0 == 1 then
			if not uv1.active then
				...
				uv1:updateRawShipsByTeam(slot2, uv4)
