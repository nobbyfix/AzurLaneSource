...
function slot0.update(slot0, slot1)
	...
	setActive(slot0.resetTimeBtn, slot0.commanderVO:getPt() > 0 or slot3 < slot1.abilityTime + CommanderConst.RESET_TALENT_WAIT_TIME)
	setActive(slot0.resetTimeTF, slot0.commanderVO:getPt() > 0 or slot3 < slot1.abilityTime + CommanderConst.RESET_TALENT_WAIT_TIME)

	if slot1.abilityTime + CommanderConst.RESET_TALENT_WAIT_TIME <= slot3 then
	...
	if not slot0.commanderVO.getNotLearnedList(slot1) or #slot2 == 0 then
		...
		slot0.usageList:make(function (slot0, slot1, slot2)
			if slot0 == UIItemList.EventUpdate then
				...
				GetImageSpriteFromAtlasAsync("CommanderTalentIcon/" .. uv0[slot1 + 1].getConfig(slot3, "icon"), "", slot2)
