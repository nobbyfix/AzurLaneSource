...
function slot0.updateEventItems(slot0, slot1, slot2)
	...
	slot0.uilist:make(function (slot0, slot1, slot2)
		if uv0(slot3) == uv1.STATE_LOCK then
		...
		elseif slot4 == uv1.STATE_ACTIVE then
			...
			slot7 = uv3[slot3].finishTime + 2

			if uv2.timers[uv3[slot3].id] then
		...
		if slot0 == UIItemList.EventUpdate then
			function slot5()
				...
				if uv1:hasCondition() then
					setText(uv0:Find("unlock/desc/task_bg/Text"), getProxy(TaskProxy):getTaskById(uv1:getTaskId()) or getProxy(TaskProxy):getFinishTaskById(slot0):getConfig("desc") .. "(" .. getProxy(TaskProxy).getTaskById(uv1.getTaskId()) or getProxy(TaskProxy).getFinishTaskById(slot0):getProgress() .. "/" .. getProxy(TaskProxy).getTaskById(uv1.getTaskId()) or getProxy(TaskProxy).getFinishTaskById(slot0):getConfig("target_num") .. ")")
				end
			end
