...
function slot0.updateMail(slot0, slot1)
	...
	if slot0.mailTFsById[slot1.id] then
		onButton(slot0, slot2, function ()
			setActive(findTF(uv0, "check_mark"), true)

			if uv1.preCheckMark and uv1.preCheckMark ~= slot0 then
				setActive(uv1.preCheckMark, false)
			end

			uv1.preCheckMark = findTF(uv0, "check_mark")

			uv1:emit(MailMediator.ON_OPEN, uv2.id)
			onButton(uv1, uv1.preCheckMark, function ()
				uv0:setOrMovePanelState("initial")

				uv0.lastOpenMailId = nil

				setActive(uv0.preCheckMark, false)
			end, SFX_PANEL)
		end, SFX_PANEL)
		...
		setText(slot8, os.date("%m/%d/%y", slot1.date))
