...
function slot0.load(slot0, slot1, slot2, slot3, slot4)
	...
	slot5:StartPreload(function ()
		...
		function slot0(slot0)
			...
			uv0._FXAttachPoint.transform.SetParent(slot2, slot0.transform, false)

			uv0._FXAttachPoint.transform.localPosition = Vector3.zero
			uv0._FXAttachPoint.transform.localEulerAngles = Vector3(330, 0, 0)
			slot5 = {
				[slot9] = Vector3(pg.ship_skin_template[uv1.skinId].fx_container[slot9][1], pg.ship_skin_template[uv1.skinId].fx_container[slot9][2], pg.ship_skin_template[uv1.skinId].fx_container[slot9][3])
			}
...
function slot0.createEmitterCannon(slot0, slot1, slot2, slot3)
	return ys.Battle.BattleBulletEmitter.New(function (slot0, slot1, slot2, slot3, slot4)
		...
		slot5:SetTemplate(uv0.shipVO.configId, slot6)

		slot8 = ys.Battle.BattleDataFunction.CreateBattleBulletData(uv1, uv1, slot5, slot7, uv2 + Vector3(40, 0, 0))
...
function slot0.createEmitterAir(slot0, slot1, slot2, slot3)
	return ys.Battle.BattleBulletEmitter.New(function (slot0, slot1, slot2, slot3, slot4)
		slot5 = {
			id = uv0,
			tmpData = pg.aircraft_template[uv0]
		}
		slot8 = Vector3(math.cos(slot7), 0, math.sin(slot7))

		function slot9(slot0)
			slot0.transform.localPosition = uv0 + Vector3(uv1.position_offset[1] + uv2, uv1.position_offset[2], uv1.position_offset[3] + uv3)
			slot0.transform.localScale = Vector3(0.1, 0.1, 0.1)
			uv4.obj = slot0
			uv4.tf = slot0.transform
			uv4.pos = uv0 + Vector3(uv1.position_offset[1] + uv2, uv1.position_offset[2], uv1.position_offset[3] + uv3)
		...
		slot10 = pg.aircraft_template[uv0].model_ID
...
function slot0.SeaUpdate(slot0)
	...
	pg.TimeMgr.GetInstance():AddBattleTimer("barrageUpdateTimer", -1, 0.033, function ()
		for slot3 = #uv0.bulletList, 1, -1 do
			if uv1 < uv0.bulletList[slot3].GetPosition(slot4).x and uv0.bulletList[slot3]._bulletData:GetSpeed()().x > 0 or slot6.z < uv2 and uv0.bulletList[slot3]._bulletData.GetSpeed()().z < 0 then
