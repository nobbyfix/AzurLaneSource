...
function slot8.flush(slot0)
	...
	for slot6, slot7 in pairs(slot0._crewUnitList) do
		...
		slot1[#slot1 + 1] = {
			weight = uv1.AntiAirPowerWeight(slot8),
			rst = slot6
		}
	end
...
function slot8.Fire(slot0)
	...
	for slot5, slot6 in pairs(slot0._hitFXResIDList) do
		pg.EffectMgr.GetInstance():PlayBattleEffect(uv2.Battle.BattleFXPool:GetInstance():GetFX(slot6), slot0._host:GetPosition() + Vector3(slot7, 10, slot8), true)
	end
