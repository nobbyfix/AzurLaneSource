...
function slot5.UpdateDiveInvisible(slot0)
	slot2 = slot0._unitData:GetIFF() == uv0.FOE_CODE
	...
	if slot0._unitData:GetDiveInvisible() then
		...
		if slot2 then
			slot0:SwitchShader(uv1:GetInstance():GetShader("GRID_TRANSPARENT"), slot0:GetFactory():GetDivingFilterColor())
			slot0:updateComponentDiveInvisible()
		else
			slot0:SwitchShader(uv1:GetInstance():GetShader("SEMI_TRANSPARENT"), slot3)
		end
...
function slot5.onDetected(slot0, slot1)
	if slot0._unitData:GetDiveDetected() and slot0._unitData:GetIFF() == uv0.FOE_CODE then
		SpineAnim.CharFade(slot0._go, slot1.Data.duration, , 0.7, true)
...
function slot5.updateComponentDiveInvisible(slot0)
	...
	SetActive(slot0._arrowBarTf, not (not (slot0._unitData:GetDiveDetected() and slot0._unitData:GetIFF() == uv0.FOE_CODE) and slot0._unitData:GetDiveInvisible()))
	SetActive(slot0._HPBarTf, not (not (slot0._unitData.GetDiveDetected() and slot0._unitData.GetIFF() == uv0.FOE_CODE) and slot0._unitData.GetDiveInvisible()))
	SetActive(slot0._FXAttachPoint, not (not (slot0._unitData.GetDiveDetected() and slot0._unitData.GetIFF() == uv0.FOE_CODE) and slot0._unitData.GetDiveInvisible()))
end
...
function slot5.UpdateHpBar(slot0)
	if slot0._HPProgress and slot0._cacheHP ~= slot0._unitData:GetCurrentHP() then
