...
function slot1.MakeBloodBar(slot0, slot1)
	if slot1:GetUnitData():GetTemplate().icon_type ~= 0 then
		setImageSprite(findTF(slot0:GetHPBarPool():GetHPBar(slot0.HP_BAR_NAME), "type"), slot5, true)
		SetActive(findTF(slot0.GetHPBarPool().GetHPBar(slot0.HP_BAR_NAME), "type"), true)
	else
