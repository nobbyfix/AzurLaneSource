...
function ys.Battle.BattleOpticalSightView.Update(slot0)
	...
	slot0._sightTF.position = uv1.Battle.BattleVariable.CameraPosToUICamera(Vector3.New(slot1, 0, 5 + slot0._fleetVO:GetMotion():GetPos().z))
	slot0._rulerTF.position = Vector3.New(0, uv1.Battle.BattleVariable.CameraPosToUICamera(Vector3.New(slot1, 0, 5 + slot0._fleetVO.GetMotion().GetPos().z)).y)
end
