...
function slot6.OpeningEffect(slot0, slot1, slot2)
	...
	if slot2 then
		...
		slot0._joystick.anchorMin = Vector2(uv0.JOY_STICK_DEFAULT_PREFERENCE.x, uv0.JOY_STICK_DEFAULT_PREFERENCE.y)
		slot0._joystick.anchorMax = Vector2(uv0.JOY_STICK_DEFAULT_PREFERENCE.x, uv0.JOY_STICK_DEFAULT_PREFERENCE.y)
...
function slot6.InitAutoBtn(slot0)
	...
	slot0._autoBtn.anchorMax = Vector2(PlayerPrefs.GetFloat("auto_anchorX", uv0.AUTO_DEFAULT_PREFERENCE.x), PlayerPrefs.GetFloat("auto_anchorY", uv0.AUTO_DEFAULT_PREFERENCE.y))
end
...
function slot6.onCastSkill(slot0, slot1)
	...
	if uv0.Battle.BattleDataFunction.GetSkillTemplate(slot3).uiEffect ~= "" then
...
function slot6.onAddUnit(slot0, slot1)
	...
	if slot1.Data.unit.IsBoss and slot3:IsBoss() then
		...
		LoadAndInstantiateAsync("UI", "MonsterAppearUI", function (slot0)
			...
			uv0._state:SetTakeoverProcess(slot2)
	...
	elseif slot1.Data.type == uv0.UnitType.ENEMY_UNIT then
		slot0:registerUnitEvent(slot3)
	elseif slot1.Data.type == uv0.UnitType.NPC_UNIT and slot3:GetIFF() == uv1.FOE_CODE then
		slot0:registerUnitEvent(slot3)
	elseif slot1.Data.type == uv0.UnitType.PLAYER_UNIT and slot3:IsMainFleetUnit() and slot3:GetIFF() == uv1.FRIENDLY_CODE then
		slot0:registerPlayerMainUnitEvent(slot3)
	end
end

function slot6.onSubmarineDetected(slot0, slot1)
	if slot0._enemyHpBar:GetCurrentTarget() and slot0._enemyHpBar:GetCurrentTarget() == slot1.Dispatcher and slot1.Dispatcher:GetDiveDetected() == false then
		slot0._enemyHpBar:RemoveUnit()
	end
end

function slot6.onRemoveUnit(slot0, slot1)
	if slot1.Data.type == uv0.UnitType.ENEMY_UNIT and not slot1.Data.unit:IsBoss() then
		slot0:unregisterUnitEvent(slot1.Data.unit)
	elseif slot2:IsMainFleetUnit() and slot2:GetIFF() == uv1.FRIENDLY_CODE then
...
function slot6.onBulletTime(slot0, slot1)
	if slot1.Data.rate then
		uv0.AppendMapFactor(slot1.Data.key, slot4)
	else
