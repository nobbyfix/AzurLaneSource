	...
	elseif slot6 == SYSTEM_SUBMARINE_RUN then
		...
		for slot22, slot23 in ipairs(slot15) do
			if slot2.statistics[slot23.id] then
				...
				slot18 = slot6 + slot5 + slot12 + slot24.id + math.floor(slot24.bp) + math.floor(slot24.output) + math.floor(slot24.maxDamageOnce)
				slot16 = 0 + slot23:getShipCombatPower()
			end
		end
		...
		pg.ConnectionMgr.GetInstance():Send(40003, {
			...
			if slot0.result == 0 or slot0.result == 1030 then
				if uv0 == SYSTEM_PERFORM or uv0 == SYSTEM_SIMULATION then
				...
				if uv0 == SYSTEM_SCENARIO and getProxy(ChapterProxy):getActiveChapter().getPlayType(slot2) == ChapterConst.TypeExtra then
					slot1 = true
				end

				if uv3.end_sink_cost > 0 and not slot1 then
					for slot6, slot7 in ipairs(uv4) do
						if uv2.statistics[slot7.id] and slot8.bp == 0 then
							slot9 = uv5:getShipById(slot7.id)

							slot9:cosumeEnergy(pg.gameset.battle_dead_energy.key_value)
							uv5:updateShip(slot9)
						end
					end
				end
				...
				if prefabFleet ~= nil then
					...
					uv1.addShipsExp(slot0.ship_exp_list, uv2.statistics, uv0 == SYSTEM_SCENARIO or uv0 == SYSTEM_ROUTINE or uv0 == SYSTEM_ACT_BOSS)

					if uv0 == SYSTEM_SCENARIO then
						slot4 = uv1:addCommanderExp(slot0.commander_exp)
						slot7 = {}
						slot8 = {}

						for slot13, slot14 in pairs(slot6) do
							table.insert({}, slot14.id)
						end
				...
				for slot9, slot10 in ipairs(slot0.drop_info) do
					...
					uv1:sendNotification(GAME.ADD_ITEM, slot11)
				end
				...
				slot2:getData().addExp(slot6, slot0.player_exp)

				if uv0 == SYSTEM_ROUTINE and ys.Battle.BattleConst.BattleScore.C < uv6 then
					slot8 = getProxy(DailyLevelProxy)
					slot8.data[slot8.dailyLevelId] = (slot8.data[slot8.dailyLevelId] or 0) + 1
				elseif uv0 == SYSTEM_ACT_BOSS and slot7 and getProxy(ActivityProxy):getActivityByType(ActivityConst.ACTIVITY_TYPE_BOSS_BATTLE) and not slot9:isEnd() then
					slot9:updateKVPList(1, uv7, slot9:getKVPList(1, uv7) + 1)
					slot9:updateDataList(uv7)
					slot8:updateActivity(slot9)
				end

				if uv0 == SYSTEM_DUEL then
					uv7 = ys.Battle.BattleConfig.ARENA_LIST[math.random(#ys.Battle.BattleConfig.ARENA_LIST)]

					getProxy(MilitaryExerciseProxy):reduceExerciseCount()
				end

				if uv3.oil_cost > 0 and slot7 then
					slot6:consume({
						oil = uv8,
						gold = uv9
					})
				...
				if uv3.attack_count > 0 and not slot1 then
					if uv3.attack_count == 1 then
						slot6:increaseAttackCount()

						if slot7 then
							slot6:increaseAttackWinCount()
						end
					elseif uv3.attack_count == 2 then
				...
				uv1:sendNotification(GAME.FINISH_STAGE_DONE, {
					...
					commanderExps = uv1.commanderExpList or {},
					result = slot0.result,
					extraDrops = slot4
				})

				if uv0 == SYSTEM_SCENARIO then
					uv10:updateActiveChapterShips()
					uv10:getActiveChapter().writeDrops(slot8, slot3)
				...
				elseif uv0 == SYSTEM_SHAM then
					uv10:updateShamChapterShips()

					slot8 = uv10:getShamChapter()

					slot8:writeBack(slot7, uv2)
					uv10:updateShamChapter(slot8)
				elseif uv0 == SYSTEM_GUILD then
					uv10:updateGuildChapterShips()

					slot8 = uv10:getGuildChapter()

					slot8:writeBack(slot7, uv2)
					uv10:updateGuildChapter(slot8)
				else
					if uv0 == SYSTEM_CHALLENGE then
						uv11:updateActiveChallengeShips()

						for slot14, slot15 in pairs(uv11:getCurrentChallengeInfo().fleet.ships) do
							function (slot0)
						...
						uv11:updateChallenge(slot8)

						return
					end

					if uv0 == SYSTEM_WORLD then
						getProxy(WorldProxy).GetWorld(slot8).GetActiveMap(slot9):WriteBack(slot7, uv2)
...
function slot0.addShipsExp(slot0, slot1, slot2)
	...
	for slot9, slot10 in ipairs(slot0) do
		if slot1[slot10.ship_id] then
			...
			if slot10.intimacy then
				slot14:addLikability(slot10.intimacy - 10000)
			end
