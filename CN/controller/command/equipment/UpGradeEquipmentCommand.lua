class("UpGradeEquipmentCommand", pm.SimpleCommand).execute = function (slot0, slot1)
	...
	if not Equipment.canUpgrade((not slot2.shipId or getProxy(BayProxy).getShipById(slot7, slot3):getEquip(slot2.pos)) and getProxy(EquipmentProxy):getEquipmentById(slot2.equipmentId).configId) then
	...
	pg.ConnectionMgr.GetInstance():Send(slot3 and 14002 or 14004, slot3 and {
		...
		if slot0.result == 0 then
			...
			slot7:consume({
				gold = (not uv0 or slot1:getShipById(uv0):getEquip(uv1)) and getProxy(EquipmentProxy):getEquipmentById(uv2).config.trans_use_gold
			})
