...
function slot0.BuildFleetPath(slot0, slot1, slot2, slot3, slot4)
	...
	_.each(slot1, function (slot0)
		...
		for slot6, slot7 in pairs(uv1.cells) do
			if not slot7.discovered and WorldConst.InFOVRange(slot0.row, slot0.column, slot7.row, slot7.column, uv1:GetFOVRange(uv2, slot0.row, slot0.column)) and not uv3[slot7.row .. "_" .. slot7.column] then
				uv3[slot7.row .. "_" .. slot7.column] = true
	...
	_.each(slot1, function (slot0)
		...
		if #_.rest(slot0.move_path, 1) > 0 then
			...
			for slot13, slot14 in ipairs(slot5) do
				slot16 = uv0:GetCell(slot14.row, slot14.column).GetTerrain(slot15)

				table.insert(, slot14)

				if slot6:GetTerrain() == WorldMapCell.TerrainWind and slot6.terrainStrong <= # or slot6:GetTerrain() ~= WorldMapCell.TerrainWind and slot16 == WorldMapCell.TerrainStream and slot16 ~= uv0.GetCell(slot1, slot2).GetTerrain(slot6) or slot6:GetTerrain() ~= WorldMapCell.TerrainWind and slot16 == WorldMapCell.TerrainIce and slot16 ~= uv0.GetCell(slot1, slot2).GetTerrain(slot6) or slot13 == #slot5 then
...
function slot0.BuildAttachmentAction(slot0, slot1)
	...
	_.each(slot1, function (slot0)
		...
		if #slot0.move_path > 0 then
			...
			slot5.attachment = uv0:GetCell(slot1, slot2).FindAliveAttachment(slot3, WorldMapAttachment.TypeEnemyAI)
