	...
	if slot0 == "call" then
		...
		elseif slot2 then
			...
			elseif slot7 ~= -1 or slot6 then
				...
				uv3((not uv5[slot5.short_src:match("([^/\\]+)%.%w+$") or slot5.short_src:match("([^/\\]+)$")] or slot10[slot7]) and (slot5.short_src.match("([^/\\]+)%.%w+$") or slot5.short_src.match("([^/\\]+)$")) .. "." .. (slot6 or slot7) or slot8 .. "." .. (slot6 or slot7), slot3, slot4)
			else
				uv3(slot2, slot3, slot4)
			end
		end
