...
function ys.Battle.BattleDataProxy.HandleAircraftMissDamage(slot0, slot1, slot2)
	...
	if #slot4 > 0 then
		slot4[math.random(#slot4)].TriggerBuff(slot6, uv1.BuffEffectType.ON_BE_HIT, {})
...
function ys.Battle.BattleDataProxy.HandleDOTPlace(slot0, slot1, slot2)
	slot5 = slot1:GetAttrByName(uv0.DOT_CONFIG[slot0.arg_list.dotType].hit)

	if uv1.IsHappen(slot0.arg_list.ACC + slot1:GetAttrByName(uv0.DOT_CONFIG[slot0.arg_list.dotType].hit) - slot2:GetAttrByName(uv0.DOT_CONFIG[slot0.arg_list.dotType].resist)) then
		return true
	end
