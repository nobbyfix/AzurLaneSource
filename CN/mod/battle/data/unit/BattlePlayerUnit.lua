...
function ys.Battle.BattlePlayerUnit.SetTemplate(slot0, slot1, slot2)
	...
	slot0._personality = uv1.Battle.BattleDataFunction.GetShipPersonality(slot0._tmpData.personality)
...
function ys.Battle.BattlePlayerUnit.setWeapon(slot0, slot1)
		...
		elseif slot9 <= Ship.WEAPON_COUNT then
			...
			function slot13(slot0, slot1, slot2)
				for slot7 = 1, uv0[uv1], 1 do
					if slot7 <= uv4 and (uv2:AddWeapon(slot0, slot1, slot2, uv3, uv1).GetTemplateData(slot8).type == uv5.POINT_HIT_AND_LOCK or uv2.AddWeapon(slot0, slot1, slot2, uv3, uv1).GetTemplateData(slot8).type == uv5.MANUAL_TORPEDO or uv2.AddWeapon(slot0, slot1, slot2, uv3, uv1).GetTemplateData(slot8).type == uv5.DISPOSABLE_TORPEDO) then
	...
	for slot11, slot12 in ipairs(slot7) do
		if slot12 and slot12 ~= -1 then
			slot0:AddWeapon(slot12, nil, nil, slot4[slot11 + #slot2] or 1, slot11 + #slot2)
		end
	end
...
function ys.Battle.BattlePlayerUnit.LeaderSetting(slot0)
	if uv0.GetWords(slot0:GetSkinID(), "hp_warning", slot0:GetDefaultSkinID()) ~= "" then
...
function ys.Battle.BattlePlayerUnit.UpdateHP(slot0, slot1, slot2, slot3, slot4)
	...
	if slot0._warningValue and slot0._currentHP < slot0._warningValue and not isHeal then
		...
		slot0:DispatchChat(uv1.GetWords(slot0:GetSkinID(), slot5, slot0:GetDefaultSkinID()), 2.5, "hp_warning")
	end
...
function ys.Battle.BattlePlayerUnit.OxyConsume(slot0)
	...
	if slot0._currentOxy <= 0 then
		slot0._fleet:ChangeSubmarineState(uv1.Battle.OxyState.STATE_FREE_FLOAT)
	end
end
