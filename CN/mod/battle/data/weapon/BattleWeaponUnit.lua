...
function slot6.createMajorEmitter(slot0, slot1, slot2, slot3, slot4, slot5)
	function slot6(slot0, slot1, slot2, slot3, slot4)
		slot6 = uv0:Spawn(slot5, slot4, uv2.INTERNAL)
	...
	if not slot3 then
		slot3 = uv0.EMITTER_NORMAL
	end

	slot8 = uv2.Battle[slot3].New
...
function slot6.IsAttacking(slot0)
	if slot0._currentState ~= uv0.STATE_ATTACK and slot0._currentState ~= slot0.STATE_PRECAST then
		slot1 = false
	else
		slot1 = true
	end
...
function slot6.FilterSquare(slot0, slot1)
	slot2 = uv0.Battle.BattleTargetChoise.TargetWeightiest(slot0._host, nil, slot1)
...
function slot6.IsOutOfAngle(slot0, slot1)
	...
	elseif slot3 < slot0._negativeCircle then
...
function slot6.PreCast(slot0)
	...
	slot0:DispatchEvent(uv0.Event.New(uv0.Battle.BattleUnitEvent.WEAPON_PRE_CAST, slot1))
end
...
function slot6.SingleFire(slot0, slot1, slot2)
	...
	if not slot2 then
		slot2 = uv0.EMITTER_NORMAL
	end

	for slot7, slot8 in ipairs(slot0._tmpData.barrage_ID) do
		slot3[#slot3 + 1] = uv1.Battle[slot2].New(slot9, function ()
...
function slot6.AddPreCastTimer(slot0)
	slot0._precastTimer = pg.TimeMgr.GetInstance():AddBattleTimer("weaponPrecastTimer", 0, slot0._preCastInfo.time, function ()
		...
		uv0:DispatchEvent(uv1.Event.New(uv1.Battle.BattleUnitEvent.WEAPON_PRE_CAST_FINISH, slot0))
		uv0:Tracking()
	end, true)
end
...
function slot6.TriggerBuffWhenSpawn(slot0, slot1, slot2)
	if not slot2 then
		slot3 = uv0.BuffEffectType.ON_BULLET_CREATE
	end
...
function slot6.DispatchBulletEvent(slot0, slot1)
	...
	slot0:DispatchEvent(uv0.Event.New(uv0.Battle.BattleUnitEvent.CREATE_BULLET, slot4))
end

function slot6.DispatchFireEvent(slot0, slot1, slot2)
	slot0:DispatchEvent(uv0.Event.New(uv0.Battle.BattleUnitEvent.FIRE, slot3))
end
...
function slot6.FlushReloadRequire(slot0)
	...
	slot0._reloadRequire = pg.TimeMgr.GetInstance():GetCombatTime() - slot0._CDstartTime + uv0.ReloadTime(slot4, slot0._host:GetAttr())
end
