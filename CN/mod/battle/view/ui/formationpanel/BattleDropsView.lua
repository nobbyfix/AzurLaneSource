...
function slot2.ShowDrop(slot0, slot1)
	...
	if slot6 > 0 then
		slot0:makeFloatAnima(Vector3(uv0.Battle.BattleVariable.CameraPosToUICamera(slot1.scenePos:Clone()).x, uv0.Battle.BattleVariable.CameraPosToUICamera(slot1.scenePos.Clone()).y, 2), slot0._resourcePool, slot0._resourceIconX, slot0._resourceIconY, slot0._resourceIcon, "_resourceCount", slot6 * uv1.RESOURCE_STEP, slot0._resourceText, 0)
	end
...
function slot2.makeFloatAnima(slot0, slot1, slot2, slot3, slot4, slot5, slot6, slot7, slot8, slot9)
	...
	LeanTween.moveX(rtf(slot10), slot10.transform.anchoredPosition.x + math.random() * 200 - 100, uv0.RESOURCE_STAY_DURATION + slot9 * 0.05):setOnComplete(System.Action(function ()
		...
		LeanTween.move(rtf(uv0), slot0, uv1.FLOAT_DURATION):setOnComplete(System.Action(function ()
			uv0:push(uv1, uv2)
