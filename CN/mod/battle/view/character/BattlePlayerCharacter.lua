...
function slot4.AddArrowBar(slot0, slot1)
	...
	setImageSprite(findTF(slot0._arrowBar, "icon"), uv1.Battle.BattleResourceManager:GetInstance().GetCharacterQIcon(slot2, slot0._unitData:GetTemplate().painting))
...
function slot4.onTorpedoPrepar(slot0, slot1)
	...
	slot0._torpedoTrack:SetScale(Vector3(uv0.Battle.BattleDataFunction.GetBulletTmpDataFromID(slot1.Dispatcher:GetTemplateData().bullet_ID[1]).range / uv1.SPINE_SCALE, uv0.Battle.BattleDataFunction.GetBulletTmpDataFromID(slot1.Dispatcher.GetTemplateData().bullet_ID[1]).cld_box[3] / uv1.SPINE_SCALE, 1))
end
...
function slot4.onTorepedoReady(slot0, slot1)
	...
	for slot6 = 1, uv0.Battle.BattleConst.MAX_EQUIPMENT_COUNT, 1 do
