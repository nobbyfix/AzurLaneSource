...
function slot2.UpdateHpBar(slot0)
	...
	elseif slot0._cacheHP < slot1 then
		while slot2.upperBound < slot1 do
			...
			if slot0._currentIndex > 5 then
				slot0._bossBarInfoList[slot0._currentIndex].deltaTF:SetSiblingIndex(slot0._topBarIndex)
				slot0._bossBarInfoList[slot0._currentIndex].progressTF:SetSiblingIndex(slot0._topBarIndex)
			else
...
function slot2.onWeaponPrecastFinish(slot0, slot1)
	...
	if slot0._castClock:GetCastingWeapon() == slot1.Dispatcher and slot1.Data.armor and slot1.Data.armor ~= 0 then
...
function slot2.OnUpdateHP(slot0, slot1)
	...
	if slot0._armor and slot1.Data.dHP < 0 then
		slot0._armor = slot0._armor + slot1.Data.dHP
