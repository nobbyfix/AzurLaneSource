...
function slot0.enterAnim(slot0)
	...
	LeanTween.moveLocalX(go(slot0.mainTF), 80, uv0):setFrom(30)
	LeanTween.moveLocalX(go(slot0.pagesTF), 0, uv0):setFrom(876):setOnComplete(System.Action(function ()
	end))
end

function slot0.exitAnim(slot0, slot1)
	LeanTween.moveLocalX(go(slot0.pagesTF), 876, uv0):setFrom(0):setOnComplete(System.Action(function ()
		uv0()
	end))
end
...
function slot0.didEnter(slot0)
	...
	onButton(slot0, slot0.modifyNameBtn, function ()
		if not uv0.commanderVO:canModifyName() then
			uv0:openMsgBox({
				content = i18n("commander_rename_coldtime_tip", uv0.commanderVO:getRenameTimeDesc())
			})
		else
			uv0:opeRenamePanel(uv0.commanderVO)
		end
	end, SFX_PANEL)
...
function slot0.checkFirstHelp(slot0)
	...
	if slot0.page == uv0.PAGE_DETAIL then
		checkFirstHelpShow("help_commander_info")
	elseif slot0.page == uv0.PAGE_PLAY then
...
function slot0.updateLockState(slot0)
	...
	onButton(slot0, slot0.lockTF, function ()
		uv1:emit(CommanderInfoMediator.ON_LOCK, uv1.commanderVO.id, 1 - uv0)
	end, SFX_PANEL)
end
