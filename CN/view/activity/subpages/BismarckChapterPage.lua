...
function slot0.InitInteractable(slot0)
	...
	for slot5, slot6 in ipairs(slot0.finalTasks) do
		...
		onButton(slot0, slot0.items[slot5], function ()
			if (uv0:getTaskById(uv1) or uv0:getFinishTaskById(uv1)) and slot0:isFinish() and not slot0:isReceive() then
...
function slot0.UpdateTab(slot0)
	...
	for slot8 = 1, #slot0.subtasks[slot0.tabType], 1 do
		...
		setActive(slot9:Find("tip2"), uv0.TransformType[pg.task_data_template[slot1[slot8]].sub_type] == DROP_TYPE_FURNITURE)
		...
		slot4 = 0 + ((slot3.getTaskById(slot10) or slot3.getFinishTaskById(slot10)) and slot3.getTaskById(slot10) or slot3.getFinishTaskById(slot10):isFinish() and 1 or 0)
	end
	...
	slot0.tab.transform.anchoredPosition.x = uv0.tabPos[slot0.tabType]
