...
function slot0.OnFirstFlush(slot0)
	...
	slot0.uilist:make(function (slot0, slot1, slot2)
		if slot0 == UIItemList.EventUpdate then
			updateDrop(uv0:findTF("item", slot2), slot8)
			onButton(uv0, uv0.findTF("item", slot2), function ()
				uv0:emit(BaseUI.ON_DROP, uv1)
			end, SFX_PANEL)
			setText(uv0:findTF("description", slot2), uv0.taskProxy:getTaskById(uv0.taskGroup[uv0.nday][slot1 + 1]) or uv0.taskProxy:getFinishTaskById(slot5):getConfig("desc"))
			setText(uv0:findTF("progressText", slot2), slot9 .. "/" .. slot10)
			setSlider(uv0:findTF("progress", slot2), 0, slot10, slot9)
			setActive(slot11, not uv0.taskProxy.getTaskById(uv0.taskGroup[uv0.nday][slot1 + 1]) or uv0.taskProxy.getFinishTaskById(slot5):isFinish())
			setActive(uv0:findTF("get_btn", slot2), uv0.taskProxy.getTaskById(uv0.taskGroup[uv0.nday][slot1 + 1]) or uv0.taskProxy.getFinishTaskById(slot5):isFinish() and not uv0.taskProxy.getTaskById(uv0.taskGroup[uv0.nday][slot1 + 1]) or uv0.taskProxy.getFinishTaskById(slot5):isReceive())
			setActive(uv0:findTF("got_btn", slot2), uv0.taskProxy.getTaskById(uv0.taskGroup[uv0.nday][slot1 + 1]) or uv0.taskProxy.getFinishTaskById(slot5):isReceive())
