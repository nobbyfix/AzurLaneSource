	...
	return setmetatable({
		...
		CreatNewMap = function (slot0, slot1)
			...
			for slot5, slot6 in ipairs(slot1) do
				...
				slot0:Notify(uv0, slot0:CreateCell(slot6[1], slot6[2], slot6[3]))
			end
		end,
		GenRandomMap = function (slot0)
			return slot0.randomer:RandomMap(uv0[slot0.id].theme[1], uv0[slot0.id].theme[2], uv1, uv2)
		end,
	...
	function slot5(slot0, slot1, slot2)
		...
		if slot2 == 0 then
			if not slot0.map.isFinished then
				slot0.onFirstFinished(slot0.map.id, slot0.map:CalcScore())

				slot0.highestScoreTxt.text = slot0.map.CalcScore()
			end
	...
	function slot2(slot0)
		eachChild(slot0, function (slot0)
			if go(slot0).name ~= "text" and go(slot0).activeSelf then
				slot0:GetComponent(typeof(Image)).color = uv0

				table.insert(uv1.selecteds, slot0.GetComponent(typeof(Image)))
			end
		end)
	end
