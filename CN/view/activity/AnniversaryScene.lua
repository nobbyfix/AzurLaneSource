	...
	for slot6 = 0, slot0:getRow(#slot0.configData) - 1, 1 do
		for slot10 = 0, uv0 - 1, 1 do
			if slot6 % 2 == 0 == (slot10 % 2 == 0) then
				cloneTplTo(slot0.taskGorupTpl, slot0.taskGorupContainer).localPosition = Vector2(slot0.offset.x * slot10, slot0.offset.y * slot6 * -1)
...
function slot0.updateTaskGroup(slot0, slot1, slot2, slot3)
	...
	if slot0.date < slot2 then
		...
		setText(slot1:Find("mask_lock"):Find("Text"), pg.TimeMgr.GetInstance():DescClientTime(slot11, "%m/%d"))
	else
...
function slot0.updateTaskGroupDesc(slot0, slot1)
	...
	function slot8(slot0, slot1)
		setText(slot0:Find("name"), uv0:getTaskById(slot1) or Task.New({
			id = slot1
		}):getConfig("name"))
		setText(slot0:Find("desc"), uv0.getTaskById(slot1) or Task.New():getConfig("desc"))
		...
		setActive(slot0:Find("confirm_btn/go"), not uv0.getTaskById(slot1) or Task.New():isFinish())
		setActive(slot0:Find("confirm_btn/finished"), uv0.getTaskById(slot1) or Task.New():isReceive())
		setActive(slot0:Find("confirm_btn/get"), uv0.getTaskById(slot1) or Task.New():isFinish() and not uv0.getTaskById(slot1) or Task.New():isReceive())
		updateDrop(setActive, {
			type = uv0.getTaskById(slot1) or Task.New():getConfig("award_display")[1][1],
			id = uv0.getTaskById(slot1) or Task.New().getConfig("award_display")[1][2],
			count = uv0.getTaskById(slot1) or Task.New().getConfig("award_display")[1][3]
		})
		onButton(uv0, setActive, function ()
			slot0 = nil
...
function slot0.moveToTaskGroup(slot0, slot1, slot2, slot3)
	...
	if slot0.date < slot1 then
		LeanTween.moveLocal(go(slot0.taskGorupContainer), Vector3(slot7, slot6, 0), 0.2):setOnComplete(System.Action(function ()
			uv0.dateIndex = uv1
		end))
...
function slot0.openAnim(slot0, slot1, slot2)
	...
	table.insert(slot3, function (slot0)
		...
		LeanTween.rotate(go(uv0), Vector3(slot1, 0, slot2), 0.2):setFrom(Vector3(slot1, -180, uv0.eulerAngles.z)):setOnComplete(System.Action(slot0))
	end)
	table.insert(slot3, function (slot0)
