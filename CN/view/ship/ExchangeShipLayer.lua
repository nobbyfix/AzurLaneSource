...
function slot0.updateFlagShipChangeTime(slot0)
	...
	if slot2 < 86400 then
		slot0.downCountTimer = Timer.New(function ()
			uv1 = pg.TimeMgr:GetInstance():DescCDTime(uv0.flagShipTime - pg.TimeMgr.GetInstance():GetServerTime())

			if uv0.flagShipTime - pg.TimeMgr.GetInstance().GetServerTime() > 0 then
