...
function slot0.onSelectCommander(slot0, slot1)
	...
	for slot10, slot11 in ipairs(slot6) do
		if slot3:getCommanderByPos(slot0) and slot11 == slot3.getCommanderByPos(slot0).id then
	...
	pg.m02:sendNotification(GAME.GO_SCENE, SCENE.COMMANDROOM, {
		...
		onSelected = function (slot0, slot1)
			if not uv0 or uv0.id ~= slot0[1] then
				for slot7, slot8 in pairs(uv1) do
					if slot8.groupId == getProxy(CommanderProxy):getCommanderById(slot0[1]).groupId and slot7 ~= uv2 and slot2 ~= slot8.id then
	...
	if getProxy(FleetProxy).EdittingFleet == nil or slot2:isFirstFleet() or slot2:isLegalToFight() == true or #slot2.ships == 0 then
		...
		pg.MsgboxMgr.GetInstance():ShowMsgBox({
			...
			onYes = function ()
				...
				for slot7 = #uv0.ships, 1, -1 do
					uv0:removeShip(getProxy(BayProxy).getRawData(slot0)[slot2[slot7]])
				end
