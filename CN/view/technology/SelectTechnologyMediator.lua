...
function slot0.onBlueprintNotify()
	...
	if not slot0:getBuildingBluePrint() then
		...
		return _.any(slot2:getTaskIds(), function (slot0)
			return uv0:getTaskStateById(slot0) == (ShipBluePrint.TASK_STATE_OPENING and getProxy(TaskProxy):isFinishPrevTasks(slot0)) or uv0.getTaskStateById(slot0) == ShipBluePrint.TASK_STATE_ACHIEVED
		end)
	end
