...
function slot0.initAddExpInfo(slot0)
	...
	for slot10, slot11 in pairs(slot0.newShipVOs) do
		slot0.shipDescTF[slot11.id] = cloneTplTo(slot0:getTpl("ship_tpl", setActive), slot4)
	...
	if slot0.dormVO.food == 0 then
		...
		while 0 < slot0:findTF("ship_word/text_contain1").childCount do
			setText(slot11:GetChild(slot12), slot10[slot12 + 1])

			slot12 = slot12 + 1
		end

		while 0 < slot0:findTF("ship_word/text_contain2").childCount do
...
function slot0.updateShip(slot0, slot1)
	...
	TweenValue(slot4, 0, slot7, uv0 * (slot1.level - slot3.level + 1), 0, function (slot0)
