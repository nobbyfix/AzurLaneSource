...
function slot0.updateFinalPanel(slot0)
	...
	slot2:make(function (slot0, slot1, slot2)
		if slot0 == UIItemList.EventUpdate then
			...
			setImageSprite(slot4, slot5)
			LoadSpriteAsync("herohrzicon/" .. uv0[slot1 + 1].shipVO:getPainting(), function (slot0)
				if not uv0.exited and uv1 == uv2[uv3 + 1] then
					setImageSprite(uv4, slot0, true)
				end
			end)
			setText(slot2:Find("black/ticket_number"), uv0[slot1 + 1].votes)
			onButton(uv1, slot2, function ()
...
function slot0.updateRacePanel(slot0)
	...
	slot2:make(function (slot0, slot1, slot2)
		if slot0 == UIItemList.EventUpdate then
			...
			setText(slot2:Find("name"), uv0[slot1 + 1].shipVO:getName())
