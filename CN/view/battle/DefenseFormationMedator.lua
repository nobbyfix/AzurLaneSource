...
function slot0.configDockYardFunc(slot0, slot1, slot2, slot3, slot4, slot5)
	return function (slot0, slot1)
		...
		if #slot0 > 0 then
			...
			elseif uv1 == Fleet.VANGUARD then
				slot3(uv3)
			...
			pg.MsgboxMgr.GetInstance():ShowMsgBox({
				...
				onYes = function ()
					if uv0 == Fleet.MAIN then
						uv1(uv2)

						uv3.mainShips = uv4
						uv3.vanguardShips = uv5
					elseif uv0 == Fleet.VANGUARD then
						uv1(uv5)
		...
		if uv2 == Fleet.MAIN then
			if not function (slot0)
				for slot4, slot5 in ipairs(slot0) do
					if (not uv1 or uv1 ~= slot5 or pg.ship_data_template[uv0[slot5].configId].group_type ~= uv2) and pg.ship_data_template[uv0[slot5].configId].group_type == uv2 then
						return false
					end
				end
		...
		elseif uv2 == Fleet.VANGUARD and not slot4(uv4) then
