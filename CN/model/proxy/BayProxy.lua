	...
	slot0:on(12010, function (slot0)
		...
		for slot7, slot8 in ipairs(slot0.ship_list) do
			if Ship.New(slot8):getConfigTable() then
				...
				if getProxy(PlayerProxy).getInited(slot1) then
					slot3 = 0 + 1
				end
...
function slot0.removeShipById(slot0, slot1)
	...
	slot0.facade:sendNotification(uv0.SHIP_REMOVED, slot0.data[slot1])
end
...
function slot0.getBayPower(slot0)
	for slot6, slot7 in pairs(slot0.data) do
		...
		if defaultValue(slot0.handbookTypeAssign[slot7:getGroupId()], 0) ~= 1 and (not ({
			[slot7.configId] = slot9
		})[slot7.configId] or ()[slot7.configId] < slot9) then
			slot2 = 0 - defaultValue(()[slot7.configId], 0) + slot9
		end
	end
...
function slot0.getEliteRecommendShip(slot0, slot1, slot2)
	...
	while #slot3 > 0 do
		if not table.contains(slot6, slot3[slot7].id) and not table.contains(slot2, slot10) and not table.contains(slot5, slot3[slot7].getGroupId(slot9)) then
			slot8 = slot9
...
function slot0.getChallengeRecommendShip(slot0, slot1, slot2)
	...
	for slot9, slot10 in ipairs(slot2) do
		if ({
			[slot12] = 0
		})[Challenge.shipTypeFixer(slot0.data[slot10]:getShipType())] == nil then
		end
...
function slot0.getActivityRecommendShips(slot0, slot1, slot2, slot3)
	...
	if #slot4 > 0 then
		while #slot4 > 0 and slot3 > 0 do
			if not table.contains(slot2, slot4[slot8].id) and not table.contains(slot7, slot11) and not table.contains(slot6, slot4[slot8].getGroupId(slot10)) then
				table.insert(slot9, slot10)
				table.insert(slot6, slot4[slot8].getGroupId(slot10))
...
function slot0.getDelegationRecommendShips(slot0, slot1)
	...
	function ()
		while #uv0 > 0 do
			...
			if uv2 <= uv0[slot0].level and not slot1:isActivityNpc() and slot1.lockState ~= Ship.LOCK_STATE_UNLOCK and not table.contains(uv3, uv0[slot0].getGroupId(slot1)) and not table.contains(uv4, uv0[slot0].id) and not table.contains(uv5, uv0[slot0].id) and not table.contains(uv6, uv0[slot0].id) and not table.contains(uv7, uv0[slot0].id) and not table.contains(uv8, uv0[slot0].id) and not table.contains(uv9, uv0[slot0].id) then
				table.insert(uv3, uv0[slot0].getGroupId(slot1))
				table.insert(uv5, uv0[slot0].id)
				table.insert(uv9, uv0[slot0].id)
