pg = pg or {}
slot0 = math.max
slot1 = math.min
slot3 = pg.CldNode
slot4 = table
pg.CldArea = class("CldArea")

function pg.CldArea.Ctor(slot0, slot1, slot2, slot3)
	slot0.min = slot1
	slot0.max = slot2
	slot0.center = slot1 + slot2:Mul(0.5)
	slot0.father = slot3

	if slot3 then
		slot0.level = slot3.level + 1
	else
		slot0.level = 1
	end

	slot0.isLeaf = true
	slot0.childs = {}
	slot0.nodes = {}
end

function pg.CldArea.AddNode(slot0, slot1)
	uv0.insert(slot0.nodes, slot1)

	slot1.area = slot0
end

function pg.CldArea.InArea(slot0, slot1, slot2)
	if slot1.x < slot0.min.x or slot1.y < slot0.min.y then
		return false
	end

	if slot0.max.x < slot2.x or slot0.max.y < slot2.y then
		return false
	end

	return true
end

function pg.CldArea.GetAreaIndex(slot0, slot1, slot2)
	if slot0.center.x <= slot1.x then
		slot4 = 0
	else
		slot4 = 2
	end

	if slot3.x <= slot2.x then
		slot5 = 0
	else
		slot5 = 2
	end

	if slot4 ~= slot5 then
		return 0
	end

	return slot4 + (slot3.z <= slot1.z and 1 or 2) == slot5 + (slot3.z <= slot2.z and 1 or 2) and slot4 + (slot3.z <= slot1.z and 1 or 2) or 0
end

slot5 = class("ColliderTree")
pg.ColliderTree = slot5
slot5.MaxLayer = 3
slot6 = 6

function slot5.Ctor(slot0, slot1, slot2, slot3, slot4)
	slot0.name = slot1
	slot0.root = uv0.CldArea.New(slot2, slot3, nil)
	slot0.MaxLayer = slot4
	slot0.cldStack = {}
end

function slot5.Insert(slot0, slot1)
	if slot1.area then
		uv0.removebyvalue(slot2.nodes, slot1)
	end

	slot0:_insert(slot1, slot0:_findParent(slot1, slot0.root))
end

function slot5._findParent(slot0, slot1, slot2)
	slot3 = slot1.min
	slot4 = slot1.max
	slot5 = nil

	while not slot2.isLeaf do
		if slot2:GetAreaIndex(slot3, slot4) < 1 then
			break
		end

		slot2 = slot2.childs[slot5]
	end

	return slot2
end

function slot5._insert(slot0, slot1, slot2)
	slot3 = nil

	if not slot2.isLeaf or #slot2.nodes < uv0 or slot0.MaxLayer <= slot2.level then
		slot2:AddNode(slot1)

		return
	end

	slot2.isLeaf = false
	slot2.childs[1] = uv1.CldArea.New(slot4, slot5, slot2)
	slot2.childs[2] = uv1.CldArea.New(Vector3(slot2.center.x, 0, slot2.min.z), Vector3(slot2.max.x, 0, slot2.center.z), slot2)
	slot2.childs[3] = uv1.CldArea.New(Vector3(slot2.min.x, 0, slot2.center.z), Vector3(slot2.center.x, 0, slot2.max.z), slot2)
	slot2.childs[4] = uv1.CldArea.New(slot6, slot4, slot2)

	for slot10 = #slot2.nodes, 1, -1 do
		if slot2:GetAreaIndex(slot2.nodes[slot10].min, slot2.nodes[slot10].max) > 0 then
			slot2.childs[slot3]:AddNode(slot11)
			uv2.remove(slot2.nodes, slot10)
		end
	end

	if slot2:GetAreaIndex(slot1.min, slot1.max) > 0 then
		slot2.childs[slot3]:AddNode(slot1)
	else
		slot2:AddNode(slot1)
	end
end

function slot5.Update(slot0, slot1)
	if slot1.area == nil then
		return
	end

	slot3 = slot1.min
	slot4 = slot1.max

	while slot2.father do
		if slot2:InArea(slot3, slot4) then
			break
		end

		slot2 = slot2.father
	end

	if slot0:_findParent(slot1, slot2) ~= slot1.area then
		uv0.removebyvalue(slot1.area.nodes, slot1)
		slot0:_insert(slot1, slot5)
	end
end

function slot5.Remove(slot0, slot1)
	if not slot1.area then
		return
	end

	uv0.removebyvalue(slot2.nodes, slot1)

	slot1.area = nil
end

function slot5.Intersect(slot0, slot1, slot2, slot3)
	if slot0.x > slot3.x or slot2.x > slot1.x or slot0.z > slot3.z or slot2.z > slot1.z then
		slot4 = false
	else
		slot4 = true
	end

	return slot4
end

function slot5.CylinderCheck(slot0, slot1)
	if not slot0.cylinder and not slot1.cylinder then
		return true
	end

	if slot0.cylinder and slot1.cylinder then
		if (slot0.center.x - slot1.center.x) * () + (slot0.center.z - slot1.center.z) * () > (slot0.range + slot1.range) * () then
			slot5 = false
		else
			slot5 = true
		end

		return slot5
	end

	if not slot0.cylinder or not slot0 then
		slot2 = slot1
	end

	slot3 = slot2.range
	slot4 = slot2.center.x
	slot5 = slot2.center.z

	if not slot0.cylinder or not slot1 then
		slot6 = slot0
	end

	if slot6.min.x <= slot4 and slot4 <= slot6.max.x then
		if slot5 < slot6.min.z - slot3 or slot5 > slot6.max.z + slot3 then
			slot7 = false
		else
			slot7 = true
		end

		return slot7
	elseif slot6.min.z <= slot5 and slot5 <= slot6.max.z then
		if slot4 < slot6.min.x - slot3 or slot4 > slot6.max.x + slot3 then
			slot7 = false
		else
			slot7 = true
		end

		return slot7
	else
		slot7, slot8 = nil

		if slot4 < slot6.min.x then
			slot7 = slot6.min.x - slot4
		else
			slot7 = slot6.max.x - slot4
		end

		if slot5 < slot6.min.z then
			slot8 = slot6.min.z - slot5
		else
			slot8 = slot6.max.z - slot5
		end

		if slot7 * slot7 + slot8 * slot8 >= slot3 * slot3 then
			slot9 = false
		else
			slot9 = true
		end

		return slot9
	end
end

function slot5.getTime(slot0, slot1, slot2)
	slot3 = 0

	if slot2.x ~= 0 then
		slot3 = uv0(0, (uv0(slot0.min.x, slot1.min.x) - uv1(slot0.max.x, slot1.max.x)) / slot2.x)
	end

	if slot2.z ~= 0 then
		slot3 = uv0(slot3, (uv0(slot0.min.z, slot1.min.z) - uv1(slot0.max.z, slot1.max.z)) / slot2.z)
	end

	return slot3
end

function slot5.GetCldList(slot0, slot1, slot2)
	slot3 = slot1.min
	slot4 = slot1.max
	slot5 = nil
	slot7 = {}

	while not slot0.root.isLeaf do
		if slot6:GetAreaIndex(slot3, slot4) < 1 then
			break
		end

		for slot11, slot12 in ipairs(slot6.nodes) do
			if uv0.Intersect(slot12.min, slot12.max, slot3, slot4) and uv0.CylinderCheck(slot1, slot12) then
				uv1.insert(slot7, slot12)
			end
		end

		slot6 = slot6.childs[slot5]
	end

	uv1.insert(slot0.cldStack, slot6)

	while #slot8 > 0 do
		for slot13, slot14 in ipairs(uv1.remove(slot8).nodes) do
			if uv0.Intersect(slot14.min, slot14.max, slot3, slot4) and uv0.CylinderCheck(slot1, slot14) then
				uv1.insert(slot7, slot14)
			end
		end

		for slot13, slot14 in pairs(slot9.childs) do
			if slot14 ~= null and uv0.Intersect(slot14.min, slot14.max, slot3, slot4) then
				uv1.insert(slot8, slot14)
			end
		end
	end

	return slot7
end

function slot5.GetCldListGradient(slot0, slot1, slot2, slot3, slot4)
	slot9 = uv0.CldNode.New()

	slot9:UpdateBox(slot10, slot11)

	if Vector3(math.cos(slot1), 0, math.sin(slot1)).x * Vector3(math.cos(slot1), 0, math.sin(slot1)).z == 0 then
		return slot0:GetCldList(slot9, nil)
	end

	slot14, slot15, slot16, slot17 = nil

	for slot21 = #slot12, 1, -1 do
		if slot13 > 0 then
			slot16 = Vector3(slot12[slot21].min.x, 0, slot12[slot21].max.z)
			slot17 = Vector3(slot12[slot21].max.x, 0, slot12[slot21].min.z)
		else
			slot14 = Vector3(slot22.min.x, 0, slot22.max.z)
			slot15 = Vector3(slot22.max.x, 0, slot22.min.z)
		end

		if Vector3.Dot(slot5, slot14 - slot7[1]) < 0 and Vector3.Dot(slot5, slot15 - slot7[1]) < 0 or slot3 < slot23 and slot3 < Vector3.Dot(slot5, slot15 - slot7[1]) then
			uv3.remove(slot12, slot21)
		elseif Vector3.Dot(slot6, slot16 - slot7[1]) < 0 and Vector3.Dot(slot6, slot17 - slot7[1]) < 0 or slot2 < slot25 and slot2 < Vector3.Dot(slot6, slot17 - slot7[1]) then
			uv3.remove(slot12, slot21)
		end
	end

	return slot12
end
