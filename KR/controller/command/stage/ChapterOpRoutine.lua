...
function slot0.doMapUpdate(slot0)
	...
	if #slot0.data.map_update > 0 then
		...
		slot2 = bit.bor(slot0.flag, bit.bor(ChapterConst.DirtyAttachment, ChapterConst.DirtyAutoAction))
	end
...
function slot0.doAIUpdate(slot0)
	...
	if #slot0.data.ai_list > 0 then
		...
		slot2 = bit.bor(slot0.flag, bit.bor(ChapterConst.DirtyChampion, ChapterConst.DirtyAutoAction))
	end
end
...
function slot0.doKizunaJammingUpdate(slot0)
	...
	if #slot0.data.add_flag_list > 0 or #slot1.del_flag_list > 0 then
		slot3:updateExtraFlag(slot0.chapter, slot1.add_flag_list, slot1.del_flag_list)
		slot3:updateChapter(slot0.chapter, ChapterConst.DirtyStrategy)
	end
end
...
function slot0.doRetreat(slot0)
	...
	if slot0.op.id then
		if #slot0.chapter.fleets > 0 then
			_.each(slot0.chapter.fleets, function (slot0)
				if slot0.id ~= uv0.id then
					table.insert(uv1, slot0)
				end
			end)

			slot0.chapter.fleets = {}
			slot0.chapter.findex = 1
...
function slot0.doAmbush(slot0)
	if slot0.op.arg1 == 1 then
		if slot2:getChapterCell(slot0.chapter.fleet.line.row, slot0.chapter.fleet.line.column).flag == 2 then
...
function slot0.doBarrier(slot0)
	...
	if slot0.chapter.getChapterCell(slot3, slot0.op.arg1, slot0.op.arg2).attachment ~= ChapterConst.AttachBox or slot4.attachmentId ~= _.detect(pg.box_data_template.all, function (slot0)
		return pg.box_data_template[slot0].type == ChapterConst.BoxBarrier
	end) then
		slot4.attachment = slot5
		slot4.attachmentId = _.detect(pg.box_data_template.all, function (slot0)
			return pg.box_data_template[slot0].type == ChapterConst.BoxBarrier
		end)
		slot4.flag = 1
	end
