...
slot7.magnitude = ({
	...
	Normalize = function (slot0)
		if uv0(slot0.x *  + slot0.y * ) > 1e-05 then
			slot1 = slot1 / slot3
			slot2 = slot2 / slot3
		else
			slot1 = 0
	...
	Angle = function (slot0, slot1)
		...
		if uv0(slot1.x *  + slot1.y * ) > 1e-05 then
			slot5 = slot5 / slot4
			slot6 = slot6 / slot4
		else
			slot6 = 0
	...
	MoveTowards = function (slot0, slot1, slot2)
		if (slot1.x - slot0.x) * () + (slot1.y - slot0.y) * () > slot2 * slot2 and slot7 ~= 0 then
			return uv1({
				x = slot3 + slot5 * slot2 / uv0(slot7),
				y = slot4 + slot6 * slot2 / uv0(slot7)
			}, uv2)
		end
	...
	ClampMagnitude = function (slot0, slot1)
		if slot0.x *  + slot0.y *  > slot1 * slot1 then
			return uv1({
				x = slot2 * slot1 / uv0(slot4),
				y = slot3 * slot1 / uv0(slot4)
			}, uv2)
		end
	...
	SmoothDamp = function (slot0, slot1, slot2, slot3, slot4, slot5)
		...
		slot7 = 1 / (1 + 2 / uv0.max(0.0001, slot3) * slot5 + 0.48 * 2 / uv0.max(0.0001, slot3) * slot5 * 2 / uv0.max(0.0001, slot3) * slot5 + 0.235 * 2 / uv0.max(0.0001, slot3) * slot5 * 2 / uv0.max(0.0001, slot3) * slot5 * 2 / uv0.max(0.0001, slot3) * slot5)

		if (slot0.x - slot1.x) * () + (slot0.y - slot1.y) * () > (slot4 or uv0.huge) * uv0.max(0.0001, slot3) *  then
			slot12 = slot12 * slot15 / uv1(slot14)
			slot13 = slot13 * slot15 / uv1(slot14)
		end
