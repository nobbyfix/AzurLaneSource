...
function slot0.register(slot0)
	...
	slot0:on(17002, function (slot0)
		for slot4, slot5 in ipairs(slot0.progress_list) do
			...
			if uv0.trophy[slot5.id] then
				uv0.trophy[slot7].update(slot8, slot5)

				if not uv0.trophy[slot7]:isHide() and uv0.trophy[slot7].canClaimed(slot8) ~= uv0.trophy[slot7].canClaimed(slot8) then
					slot6 = true
				end
			else
	...
	slot0:on(17004, function (slot0)
		uv0.shipGroups[slot0.ship_info.id] = ShipGroup.New(slot0.ship_info)
	end)
end
