...
function pg.ShareMgr.Share(slot0, slot1, slot2)
	if not WBManager.IsSupportShare() then
		uv0.TipsMgr.GetInstance():ShowTips(i18n("non_support_share"))
	...
	if (1.7777777777777777 >= Screen.width / Screen.height or ScreenShooter.New(math.floor(Screen.height * slot12), Screen.height, TextureFormat.ARGB32)) and ScreenShooter.New(Screen.width, math.floor(Screen.width / slot12), TextureFormat.ARGB32):Take(uv0.share_template[slot1].deck or 1, slot0.screenshot) then
		print("截图位置: " .. slot0.screenshot)
		slot0:Show(slot3)
	else
		uv0.TipsMgr:GetInstance():ShowTips(i18n("shoot_screen_fail"))
	end
	...
	onButton(slot0, slot0.panel:Find("main/buttons/weixin"), function ()
		WXManager.Inst:Share(uv0.description, uv1.screenshot, function (slot0, slot1)
			...
			elseif slot1 == 99 then
				uv0.TipsMgr:GetInstance():ShowTips(i18n("not_install_app"))
			end
		end)
		uv3()
	end)
