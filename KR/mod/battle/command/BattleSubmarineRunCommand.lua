...
function slot4.onWillDie(slot0, slot1)
	if slot1.Dispatcher.GetDeathReason(slot2) == uv0.Battle.BattleConst.UnitDeathReason.KILLED or slot3 == uv0.Battle.BattleConst.UnitDeathReason.DESTRUCT then
		for slot7, slot8 in pairs(slot0._unitDataList) do
			slot8:TriggerBuff(uv0.Battle.BattleConst.BuffEffectType.ON_FRIENDLY_SHIP_DYING, {
				unit = slot8
			})
		end
	end
	...
	uv1.super.onWillDie(slot0, slot1)
end
