...
function ys.Battle.BattleShrapnelBulletUnit.SetSpawnPosition(slot0, slot1)
	...
	if slot0:GetTemplate().extra_param.flare then
		...
		slot7 = math.abs
		slot8 = uv1.Battle.BattleDataFunction.GetBulletTmpDataFromID(slot4).extra_param.gravity or -0.0005
		slot0._convertedVelocity = math.sqrt(-0.5 * slot0._gravity * slot3 * slot3 / (0.5 * math.abs(slot8) * (uv1.Battle.BattleDataFunction.GetBulletTmpDataFromID(slot4).hit_type.time * uv1.Battle.BattleConfig.calcFPS)^2 - slot0._spawnPos.y))
		slot0._verticalSpeed = (0.5 * math.abs(slot8) * (uv1.Battle.BattleDataFunction.GetBulletTmpDataFromID(slot4).hit_type.time * uv1.Battle.BattleConfig.calcFPS)^2 - slot0._spawnPos.y) / (slot3 / slot0._convertedVelocity) - 0.5 * slot0._gravity * slot3 / slot0._convertedVelocity
