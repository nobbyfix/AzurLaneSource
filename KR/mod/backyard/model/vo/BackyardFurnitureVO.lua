...
function slot0.getSpineAinTriggerPos(slot0)
	if slot0:isInterActionSpine() then
		if slot0.dir == 1 then
			return Vector2(slot0.position.x + slot0:getInterActionSpineCfg()[1][1], slot0.position.y + slot0.getInterActionSpineCfg()[1][2])
	...
	if slot0:isArch() then
		...
		for slot7 = #slot2, 1, -1 do
			for slot12, slot13 in ipairs(slot3) do
				if slot2[slot7].x == slot13.x and slot2[slot7].y == slot13.y then
...
function slot0.getChildPosById(slot0, slot1)
	if slot0.dir == 1 then
		return Vector2(slot0.position.x + slot0.child[slot1].x, slot0.position.y + slot0.child[slot1].y)
	elseif slot0.dir == 2 then
...
function slot0.getSize(slot0)
	if slot0.dir == 1 then
		return slot0:getConfig("size")[1], slot0.getConfig("size")[2]
	else
