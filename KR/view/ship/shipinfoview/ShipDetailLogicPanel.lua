...
function slot0.attach(slot0, slot1)
	...
	slot0.evalueIndex = slot0.contextData.evalueIndex or uv0.EQUIPMENT_ADDITION
...
function slot0.updateEvalues(slot0)
	...
	for slot5, slot6 in pairs(slot0.additionValues.transforms) do
		if slot5 == AttributeType.Armor or slot5 == AttributeType.Expend or slot5 == AttributeType.HuntingRange and slot1 then
			...
			setText(slot6, (slot0.additionValues[slot0.evalueIndex][slot5] or 0) == 0 and "" or setColorStr(" +" .. (slot0.additionValues[slot0.evalueIndex][slot5] or 0), slot0.evalueIndex == uv0.EQUIPMENT_ADDITION and COLOR_GREEN or COLOR_YELLOW))
...
function slot0.updateWorldBuffs(slot0)
	...
	slot3:make(function (slot0, slot1, slot2)
		if slot0 == UIItemList.EventUpdate then
			setText(slot2:Find("level"), uv0[slot1 + 1].level .. "/" .. uv0[slot1 + 1]:GetMaxLevel())

			slot4 = uv0[slot1 + 1].config.buff_effect[1] * uv0[slot1 + 1].level

			if uv0[slot1 + 1].config.percent[1] == 1 then
				slot4 = slot4 / 100 .. "%"
			end
...
function slot0.updateShipAttrs(slot0)
	...
	for slot10, slot11 in pairs(uv1) do
		...
		slot0.additionValues[0][slot11] = LOCK_EQUIP_DEVELOPMENT and (slot4[slot11] or 0) or math.floor(((slot4[slot11] or 0) + (slot3[slot11] or 0)) * (slot5[slot11] or 1)) - (slot3[slot11] or 0)
	...
	slot3:make(function (slot0, slot1, slot2)
		if slot0 == UIItemList.EventUpdate then
			if uv0[slot1 + 1] then
				...
				if uv1.skills[uv1:fateSkillChange(slot3)] and slot5.id == 11720 and not uv1.transforms[3612] then
					slot5 = nil
				end
