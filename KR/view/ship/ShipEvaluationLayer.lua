...
function slot0.flushEva(slot0)
	...
	for slot6 = 1, #slot2, 1 do
		...
		onButton(slot0, nil:Find("bg/zan_bg/up"), function ()
			uv0(0)
		end, SFX_PANEL)
		onButton(slot0, nil:Find("bg/zan_bg/down"), function ()
			uv0(1)
		end, SFX_PANEL)
		SetActive(nil:Find("bg/zan_bg/down"), not LOCK_DOWNVOTE or false)

		if LOCK_DOWNVOTE then
			nil:Find("bg/zan_bg/up").position = nil:Find("bg/zan_bg/down").position + Vector3(0, 0.15, 0)
		end
	end
