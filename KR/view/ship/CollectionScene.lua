...
function slot0.onInitCard(slot0, slot1)
	onButton(slot0, CollectionShipCard.New(slot1).go, function ()
		...
		elseif uv0.state == ShipGroup.STATE_NOTGET and uv0.config then
			uv1:showSkipableMsgBox(uv0.config.description, uv0.shipGroup:getShipConfigId())
		end
	end, SOUND_BACK)
...
function slot0.showSkipableMsgBox(slot0, slot1, slot2)
	...
	if not slot0.skipableList then
		slot0.skipableList = UIItemList.New(slot0.skipableMsgBoxTF:Find("window/item/skipable_list"), slot4)

		onButton(slot0, findTF(slot0.skipableMsgBoxTF, "window/top/btnBack"), function ()
			uv0:closeSkipableMsgBox()
		end, SFX_PANEL)
	...
	slot0.skipableList:make(function (slot0, slot1, slot2)
		if slot0 == UIItemList.EventUpdate then
			...
			setActive(slot2:Find("skip_btn"), uv0[slot1 + 1][2][1] ~= "")

			if slot5[1] ~= "" then
				onButton(uv1, slot2:Find("skip_btn"), function ()
					if uv0 and uv0 ~= 0 and (not getProxy(ActivityProxy):getActivityById(uv0) or slot0:isEnd()) then
						pg.TipsMgr:GetInstance():ShowTips(i18n1("대응하는 획득 경로가 개방되지 않았습니다"))

						return
					end

					if uv1[1] == "LEVEL" and uv1[2] then
						uv2.contextData.cardScrollValue = uv2.cardList.value

						uv2:emit(uv3.GO_LEVEL, uv1[2].chapterid)
					elseif uv1[1] == "COLLECTSHIP" then
						slot0 = 0

						for slot5, slot6 in ipairs(uv2.favoriteVOs) do
							if slot6:containShipGroup(uv1[2].shipGroupId) then
								slot0 = slot5
						...
						if not uv2.isInitDisplay then
							uv2:initDisplayPanel()
						end

						uv2.contextData.displayPos = uv2.displayRect:HeadIndexToValue(slot0 - 1)

						triggerToggle(uv2.toggles[2], true)
						uv2:closeSkipableMsgBox()
					elseif uv1[1] == "SHOP" and uv1[2].warp == "supplies" and not pg.SystemOpenMgr:GetInstance():isOpenSystem(uv2.player.level, "MilitaryExerciseMediator") then
						pg.TipsMgr:GetInstance():ShowTips(i18n("military_shop_no_open_tip"))
					else
						uv2.contextData.cardScrollValue = uv2.cardList.value

						uv2:emit(uv3.GO_SCENE, SCENE[uv1[1]], uv1[2])
					end
				end, SFX_PANEL)
			end
		end
...
function slot0.transFilter(slot0)
	...
	if uv0.ShipIndex.index == bit.lshift(1, IndexConst.IndexAll) and uv0.ShipIndex.rarity == bit.lshift(1, IndexConst.RarityAll) and uv0.ShipIndex.camp == bit.lshift(1, IndexConst.CampAll) then
		...
		for slot7 = slot2, slot3, 1 do
			if pg.ship_data_group[slot7] and slot0.shipGroups[slot8.group_type] and slot9.trans and IndexConst.filterByIndex(slot9, uv0.ShipIndex.index) and IndexConst.filterByRarity(slot9, uv0.ShipIndex.rarity) and IndexConst.filterByCamp(slot9, uv0.ShipIndex.camp) then
				slot0.codeShips[#slot0.codeShips + 1] = {
					showTrans = true,
					id = slot7,
					code = 3000 + slot7,
