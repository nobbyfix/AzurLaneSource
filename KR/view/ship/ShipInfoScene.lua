...
function slot0.preload(slot0, slot1)
	GetSpriteFromAtlasAsync("bg/star_level_bg_" .. getProxy(BayProxy).getShipById(slot2, slot0.contextData.shipId).getShipBgPrint(slot3), "", slot1)
end
...
function slot0.updateEquipments(slot0, slot1)
	...
	for slot6, slot7 in ipairs(slot1.equipments) do
		...
		if slot7 then
			updateEquipment(slot0:findTF("IconTpl", nil), slot7)
...
function slot0.updateEquipmentPanel(slot0, slot1, slot2, slot3)
	...
	slot9:GetComponent(typeof(Image)).sprite = slot0:findTF(slot11, slot0.resource):GetComponent(typeof(Image)).sprite
				...
				elseif slot18 == true and slot22 == 4 then
					...
					if slot17[slot22] then
						if slot2.config.type ~= EquipType.Equipment and slot26.type == AttributeType.Reload and slot22 == 4 then
							...
							setText(findTF(slot24, "values/value"), slot31)
...
function slot0.updateFashion(slot0)
	setActive(slot0.fashionToggle, #slot0:getGroupSkinList(slot1) > 1)
	...
	onButton(slot0, slot2.confirm, function ()
		...
		elseif uv1 then
			if HXSet.isHx() and uv2.isSwimsuit == 1 then
				slot1 = uv3
				slot0 = uv3.loadPainting

				if uv2.painting_hx == "" or not uv2.painting_hx then
					slot2 = uv2.painting
				end
				...
				for slot3, slot4 in ipairs(uv3.fashionSkins) do
					uv3.fashionCellMap[uv3.styleContainer:GetChild(slot3 - 1)]:updateSelected(slot4.id == uv3.fashionSkinId)
...
function slot0.blurPage(slot0, slot1, slot2)
	...
	elseif slot1 == uv0.PAGE.EQUIPMENT then
		if slot2 then
			...
			local function slot6(slot0, slot1)
	...
	if slot1 == slot0.page and slot2 then
		function (slot0, slot1)
			if slot0 == uv0.PAGE.DETAIL then
				shiftPanel(uv1.detailPanel, slot1 and {
					uv1.detailPanel.rect.width + 200,
					0
				} or {
					0,
					uv1.detailPanel.rect.width + 200
				}[2], 0, uv2, 0):setFrom(slot1 and  or [1])
			elseif slot0 == uv0.PAGE.EQUIPMENT then
				shiftPanel(uv1.equipmentL, slot1 and {
				...
				shiftPanel(uv1.equipmentR, slot1 and {
			...
			elseif slot0 == uv0.PAGE.FASHION then
				shiftPanel(uv1.stylePanel, slot1 and {
	...
	if slot0.shipSkinBg ~= slot1 or slot0.isDesign ~= slot2 then
		...
		if slot0.isDesign then
			...
			for slot6 = 1, 5, 1 do
				...
				-- Decompilation error in this vicinity:
				--- BLOCK #0 73-114, warpins: 2 ---
				if slot0.bgEffect[slot6] then

					-- Decompilation error in this vicinity:
					--- BLOCK #0 81-96, warpins: 1 ---
					slot8 = setActive
					slot9 = slot0.bgEffect[slot6]

					if slot6 ~= slot0.shipVO:getRarity() or slot0.page == uv0.PAGE.REMOULD then

						-- Decompilation error in this vicinity:
						--- BLOCK #0 92-93, warpins: 2 ---
						slot10 = false
						--- END OF BLOCK #0 ---
					...
					if slot7 > 2 and slot7 == slot6 then

						-- Decompilation error in this vicinity:
						--- BLOCK #0 102-112, warpins: 1 ---
						PoolMgr.GetInstance():GetUI("al_bg02_" .. slot7 - 1, true, function (slot0)
							...
							if uv0.page == uv2.PAGE.REMOULD then
...
function slot0.updateRecordEquipments(slot0, slot1)
	...
	for slot8 = 1, 5, 1 do
		...
		setActive(slot2:Find("equipment_" .. slot8):Find("empty"), not slot10)

		if slot10 then

			-- Decompilation error in this vicinity:
			--- BLOCK #0 51-100, warpins: 1 ---
			slot14 = slot0.equipmentProxy:getEquipmentById(slot9)

			if not slot0.shipVO.equipments[slot8] or slot15.id ~= slot9 then

				-- Decompilation error in this vicinity:
				--- BLOCK #0 64-65, warpins: 2 ---
				slot16 = false

				if false then

					-- Decompilation error in this vicinity:
					--- BLOCK #0 66-67, warpins: 0 ---
					slot16 = false
					--- END OF BLOCK #0 ---
