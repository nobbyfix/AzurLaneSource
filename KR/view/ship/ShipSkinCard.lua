	...
	if slot0.ship ~= slot1 or slot0.skin ~= slot2 or slot0.own ~= slot3 then
		...
		if not (slot0.skin.id == slot0.ship.getConfig("skin_id")) and not slot3 then
			...
			elseif slot0.skin.shop_id > 0 then
				setActive(slot0.picNotBuy, true)
			elseif _.any(pg.activity_shop_template.all, function (slot0)
				if pg.activity_shop_template[slot0].commodity_type ~= DROP_TYPE_SKIN or slot1.commodity_id ~= uv0.skin.id then
					slot2 = false
				else
					slot2 = true
				end
				...
				if pg.activity_shop_extra[slot0].commodity_type ~= DROP_TYPE_SKIN or slot1.commodity_id ~= uv0.skin.id then
					slot2 = false
				else
					slot2 = true
				end
...
function slot0.loadPainting(slot0)
	...
	elseif slot0.skin.painting_hx == "" or not slot0.skin.painting_hx then
		slot1 = slot0.skin.painting
	end
