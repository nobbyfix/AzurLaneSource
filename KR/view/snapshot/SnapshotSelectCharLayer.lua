		...
		for slot6, slot7 in ipairs(slot2) do
			if pg.ship_data_group[slot7] then
				if ShipGroup.New({
					...
					if slot0.toggleType == uv0.TOGGLE_CHAR and not Nation.IsLinkType(slot9:getNation()) and IndexConst.filterByCamp(slot9, uv0.ShipIndex.camp) then
						slot1[#slot1 + 1] = {
							showTrans = false,
							code = uv1(slot0.toggleType, Nation.IsLinkType(slot9.getNation()), slot7),
