...
function slot0.createThemeItem(slot0, slot1)
	slot3 = findTF(({
		...
		update = function (slot0, slot1, slot2, slot3, slot4)
			...
			SetActive(uv4, defaultValue(SetActive, 0) > 0 and pg.TimeMgr.GetInstance():inTime(slot1:getConfig("discount_time")))
			setText(findTF(uv4, "Text"), defaultValue(SetActive, 0) .. "%off")
...
function slot0.initThemeItem(slot0, slot1)
	...
	onButton(slot0, slot2.maskLocked, function ()
		uv0:showMsgBox({
			...
			ids = uv1.themeVO:getRemainFurIds(uv0.furnitureVOs)
		}, nil, true)
	end)
