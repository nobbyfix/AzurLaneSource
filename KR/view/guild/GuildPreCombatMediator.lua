	...
	elseif not slot1.fleet:isValid() then
		...
		if slot1.active then
			if getProxy(GuildProxy):getGuildEvent() and not slot4:isEnd() then
				slot0:save(function ()
					slot1 = uv0:getChapterCell(uv0.fleet.line.row, uv0.fleet.line.column)
	...
	slot6 = coroutine.create(function ()
		...
		elseif uv0 == 2 then
			for slot13, slot14 in pairs(slot6) do
				if not getProxy(PlayerProxy).playerGuildAssists[slot13] or pg.TimeMgr.GetInstance():GetServerTime() - getProxy(PlayerProxy).playerGuildAssists[slot13].timeStamp > 60 then
					table.insert({}, slot13)
				end
			end
