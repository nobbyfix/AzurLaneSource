...
function slot0.handleNotification(slot0, slot1)
	...
	if slot1:getName() == ActivityProxy.ACTIVITY_OPERATION_DONE then
		slot6 = getProxy(ChapterProxy).getUnlockActMapBytype(slot4, slot0.activityProxy:getActivityById(slot1.getBody()).data1, slot0.activityProxy.getActivityById(slot1.getBody()).id)
