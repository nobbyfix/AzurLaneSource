...
function slot0.updateSelectedShipList(slot0)
	for slot5 = 1, 2, 1 do
		...
		if slot0.prayProxy:getSelectedShipIDList()[slot5] then
			setActive(slot0:findTF("Paint", slot7), true)
			setPaintingPrefabAsync(slot0.findTF("Paint", slot7), Ship.getPaintingName(slot6), "biandui")
...
function slot0.updateShipList(slot0, slot1)
	...
	function slot0.shipListSC.onUpdateItem(slot0, slot1)
		...
		if table.indexof(uv2, uv0[slot0 + 1], 1) then
