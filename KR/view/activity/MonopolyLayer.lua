...
function slot0.didEnter(slot0)
	...
	slot0.mapVO = uv0(setActive, slot0.pos)
	...
	onButton(slot0, slot0.advanceRp, function ()
		...
		if uv0.leftAwardCnt > 0 then
			uv0:emit(MonopolyMediator.ON_AWARD, uv0.activity.id)
		end
	end, SFX_PANEL)
	...
	seriesAsync({
		...
		function (slot0)
			...
			function ()
				...
				uv2:emit(MonopolyMediator.ON_TRIGGER, uv2.activity.id, function (slot0, slot1)
					...
					uv1.mapVO:UpdateCharPos(slot0, function ()
						uv2 = uv1.mapVO:GetCell(slot0).GetEvent(slot1, uv3)
...
function slot0.checkState(slot0)
	...
	if slot0.step ~= 0 then
		table.insert(slot1, function (slot0)
			uv0:emit(MonopolyMediator.ON_MOVE, uv0.activity.id, function (slot0, slot1, slot2)
...
function slot0.startAction(slot0)
	...
	table.insert(slot2, function (slot0)
		uv0:emit(MonopolyMediator.ON_START, uv1, function (slot0)
			uv0 = slot0
	...
	for slot7 = 1, 2, 1 do
		table.insert(slot2, function (slot0)
			...
			uv1:emit(MonopolyMediator.ON_MOVE, uv2, function (slot0, slot1, slot2)
				if not slot1 or #slot1 == 0 then
					uv0()
...
function slot0.createMap(slot0, slot1)
	...
	function slot1.onCreateChar(slot0)
		...
		uv0.charCard = uv1(cloneTplTo(uv0.charTpl, uv0.mapContainer), slot0)
