	...
	if not slot5 then
		...
		function slot8(slot0, slot1, slot2)
			...
			if slot0 == slot3 then
				...
				setActive(slot2:Find("got"), slot1 + 1 <= #uv3.data1_list)
				setActive(slot2:Find("today"), slot1 + 1 == #uv3.data1_list)
			end
		end
	...
	if not slot11 then
		...
		function slot13(slot0, slot1, slot2)
			...
			if slot0 == slot3 then
				...
				setText(slot2:Find("target/Text"), pg.task_data_template[uv0[slot1 + 1]].target_num)
...
function slot7(slot0, slot1, slot2, slot3)
	...
	if slot13 then
		...
		if slot10 < slot14 then
			setText(slot6.progressTxt, slot15)
			setSlider(slot6.progress, 0, slot14, math.min(slot10, slot14))
			updateDrop(slot6.award, slot17)
			...
			slot18 = setActive
			slot19 = slot6.btn:Find("get")

			if slot13:isFinish() then
				slot21 = slot13
