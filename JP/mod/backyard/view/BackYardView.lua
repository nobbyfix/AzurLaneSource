	...
	for slot7, slot8 in pairs(slot3) do
		table.insert({}, function (slot0)
			if not uv0.boatVO:hasSpineInterAction() then
				uv1:emit(BackyardMainMediator.CANCEL_SHIP_MOVE, uv0.boatVO.id)
				uv0:removeItem()
				uv0:updateShadowTF(false)

				if not uv0.boatVO:hasInterActionFurnitrue() then
					uv0.tf:SetAsLastSibling()
				end

				slot0()
			elseif uv1.furnitureVOs[uv0.boatVO:getSpineId()]:isLoopSpineInterAction() then
				slot0()
			else
				uv0:breakSpineAnim(slot0)
			end
		end)
	end
...
function slot0.initFurnitures(slot0)
	function slot1(slot0, slot1)
		uv0:loadFurnitureModel(slot0, function (slot0)
			if not slot0 then
				uv0()
			...
			LeanTween.scale(rtf(slot0), Vector3(slot0.localScale.x + 0.2, slot0.localScale.y + 0.2, 1), 0.2):setFrom(0):setOnComplete(System.Action(function ()
				LeanTween.scale(rtf(uv0), Vector3(uv1, uv2, 1), 0.1):setOnComplete(System.Action(uv3))
			end))
		end)
	end
	...
	seriesAsync({
		function (slot0)
			limitedParallelAsync(uv0, 5, slot0)
		end,
		function (slot0)
			limitedParallelAsync(uv0, 5, slot0)
		end,
...
function slot0.loadFurnitureModel(slot0, slot1, slot2)
	...
	if not slot1:isSpine() then
		slot0:loadImageFurniture(slot1, function (slot0)
			if not slot0 or IsNil(uv0) then
				if uv1 then
					uv1()
				end
			...
			slot3 = uv3.dir == 2
			uv0.localScale = Vector3(uv2.getSign, 1, 1)

			uv4:setWallModalDir(uv3, uv5)
			uv4:updateFurnitruePos(uv3, true)

			uv6.anchoredPosition3D = Vector3(0, 0, 0)

			uv4:registerFurnitureEvent(uv3)

			if uv4.decorateMode then
				if not IsNil(uv4.curFurnModal) then
					uv4:closePreFurnSelected()
				end
...
function slot0.loadImageFurniture(slot0, slot1, slot2)
	...
	GetSpriteFromAtlasAsync("furniture/" .. slot1:getConfig("picture"), "", function (slot0)
		...
		slot1 = uv3.createImage("icon", true, uv2, 1)

		slot1:AddComponent(typeof(AlphaCheck))
		setImageSprite(slot1, slot0, true)

		if uv4:hasInterActionMask() then
			uv0:loadFurnituresMasks(uv4, function ()
				uv0(uv1)
			end)
		elseif uv4:isArch() then
			uv0:loadArchMask(uv4, function ()
				uv0(uv1)
			end)
		else
			uv1(slot0)
...
function slot0.loadArchMask(slot0, slot1, slot2)
	...
	ResourceMgr.Inst:getAssetAsync("furniture/" .. slot4, "", typeof(Sprite), UnityEngine.Events.UnityAction_UnityEngine_Object(function (slot0)
		...
		setImageSprite(uv2.createImage(BackYardConst.ARCH_MASK_NAME, false, uv3, 2, true), slot0, true)
	...
	for slot9, slot10 in pairs(slot4) do
		table.insert({}, function (slot0)
			ResourceMgr.Inst:getAssetAsync("furniture/" .. uv0, "", typeof(Sprite), UnityEngine.Events.UnityAction_UnityEngine_Object(function (slot0)
				...
				setActive(slot1, false)
				setImageSprite(uv2.createImage(BackYardConst.FURNITRUE_MASK_ORDER_NAME .. uv3, false, uv4, 2, true), slot0, true)
				uv5()
			end), true, true)
		end)
	end
...
function slot0.loadSpineFurnitureModel(slot0, slot1, slot2)
	...
	LoadAndInstantiateAsync("sfurniture", slot4, function (slot0)
		...
		uv2.pivot = rtf(slot0).pivot

		slot3(slot1, "icon", 1, uv3)

		if uv4:hasSpineMask() then
			slot8, slot5 = uv4:getSpineMaskName()
			...
			LoadAndInstantiateAsync("sfurniture", slot4, function (slot0)
				...
				uv0:loadSpineAnimator(uv3, function ()
					uv0(uv1)
				end)
			end, true, true)
		else
			uv0:loadSpineAnimator(uv4, function ()
				uv0(uv1)
			end)
		end
	end, true, true)
...
function slot0.registerFurnitureEvent(slot0, slot1)
	...
	onButton(slot0, slot4, function ()
		if uv0.decorateMode then
			...
			if not LeanTween.isTweening(go(uv1)) then
				LeanTween.scale(uv1, Vector3(uv1.localScale.x - 0.05, uv1.localScale.y - 0.05, uv1.localScale.z - 0.05), 0.01):setOnComplete(System.Action(function ()
					LeanTween.scale(uv0, uv1, 0.1)
				end))
			end
		...
		elseif uv4:isShowDesc() then
			uv0:showFurnitrueDesc(uv4)
		elseif uv4:isTouchSpine() then
			uv0:playTouchSpineAnim(uv4, uv5)
		end
	end, SFX_PANEL)
	...
	slot8:AddDragFunc(function (slot0, slot1)
		uv1:furnitureDrag(uv0.change2ScrPos(uv1.floorGrid, slot1.position), uv2)
	end)
	slot8:AddDragEndFunc(function (slot0, slot1)
		uv0:enableZoom(true)
		uv0:furnitureEndDrag(uv1.getMapPos(slot2), uv2)
	end)
	...
	onButton(slot0, slot0:findTF("rotation", slot3), function ()
		...
		uv0:emit(BackyardMainMediator.FURNITURE_DIR_CHANGE, uv1.id)
	end, SFX_PANEL)
end
	...
	slot4:SetActionCallBack(function (slot0)
		if slot0 == "finish" and not uv0 then
			...
			if uv2:isMoveable() then
				uv3:emit(BackyardMainMediator.ON_REMOVE_MOVE_FURNITURE, uv2.id)
			end
...
function slot0.createbottomGrid(slot0, slot1, slot2)
	...
	for slot11, slot12 in ipairs(slot7) do
		slot13 = slot0:getGridTpl(slot1:isFloor())
...
function slot0.showFurnitrueDesc(slot0, slot1)
	...
	function ()
		setActive(uv0.miniPanel, false)
		setActive(uv0.maxFrame, true)

		if uv1 then
			uv0.maxIcon.sprite = uv1
		else
			LoadSpriteAsync("FurnitureIcon/" .. uv2:getConfig("icon"), function (slot0)
				if not IsNil(uv0._tf) then
		...
		uv0.maxName.text = shortenString(uv2:getConfig("name"), 6)
		uv0.maxdate.text = uv2:getDate()
		uv0.maxComfortable.text = "+" .. uv2:getConfig("comfortable")
		uv0.maxContent.text = uv2:getConfig("describe")
		uv0.maxApproach.text = uv2:getGainby()
		uv0.maxType.text = uv2:getChineseType()

		pg.UIMgr.GetInstance():BlurPanel(uv0.maxFrame)
	end()
end
...
function slot0.updateFurnitureWithAnim(slot0, slot1, slot2, slot3)
	if slot2 == 0 then
		...
		LeanTween.moveLocal(go(slot5), slot8, slot2 / 2):setOnComplete(System.Action(function ()
			uv0:removeItem(uv1)
			uv0:createItem(uv1, uv2.x, uv2.y)
			LeanTween.moveLocal(go(uv3), uv4, uv5 / 2)
		end))
	end
end
...
function slot0.removeFurn(slot0, slot1)
	...
	function slot3(slot0)
		SetParent(uv0.shipModels[slot0].tf, uv0.floorContain)
		uv0.shipModels[slot0]:setAction("stand2")
	end
	...
	if slot1:hasStageShip() then
		for slot8, slot9 in pairs(slot1:getStageShip() or {}) do
			slot3(slot9)
	...
	for slot9, slot10 in pairs(slot0.furnBottomGrids[slot1.id] or {}) do
		slot0.backyardPoolMgr:Enqueue(slot1:isFloor() and slot0.backyardPoolMgr.POOL_NAME.GRID or slot0.backyardPoolMgr.POOL_NAME.WALL, slot10)
	end
...
function slot0.furnitureDrag(slot0, slot1, slot2)
	...
	if not slot2:isFloor() and not slot0.houseVO:isLimitWallBound(slot2, slot0.houseVO:limitWallFurnWidth(slot3, slot2)) then
		return
	end
	...
	if slot0.houseVO.isLocaledAndPutOn(slot2, slot4) then
		slot0.furnitureModals[slot2.id].localPosition = Vector2(uv0.getLocalPos(slot4).x + slot7:getConfig("offset")[1], uv0.getLocalPos(slot4).y + slot7.getConfig("offset")[2])
	else
...
function slot0.furnitureEndDrag(slot0, slot1, slot2)
	...
	if not slot0.houseVO:isLegalPos(slot2, slot4) then
		...
		LeanTween.moveLocal(go(slot3), Vector3(uv0.getLocalPos(slot5).x, uv0.getLocalPos(slot5).y, 0), 0.1):setOnComplete(System.Action(function ()
			if uv0:hasParent() then
				uv2.localPosition = Vector2(uv2.localPosition.x + uv1.furnitureVOs[uv0.parent].getConfig(slot0, "offset")[1], uv2.localPosition.y + uv1.furnitureVOs[uv0.parent].getConfig(slot0, "offset")[2])

				uv2:SetParent(uv1.furnitureModals[uv0.parent].Find(slot3, "childs"), true)
				uv1.maps[uv0.parent].afterSortFunc(uv1.maps[uv0.parent].sortedItems)
			end

			uv1.map.afterSortFunc(uv1.map.sortedItems)
			uv1:setPreSelectedParent(uv1.furContain)

			if not uv0:isFloor() then
				uv1:sortWallFurns()
			end

			uv1.decoratePanelCG.blocksRaycasts = true
			uv1.isDraging = nil
		end))
...
function slot0.rotateFurn(slot0, slot1)
	...
	if slot1:hasInterActionShipId() then
		for slot7, slot8 in ipairs(slot3) do
			slot0.shipModels[slot8]:updateExpTFScale(uv0.getSign(slot2.localScale.x * slot0.shipModels[slot8].tf.localScale.x < 0))
		end
	elseif slot1:getSpineId() then
		slot0.shipModels[slot1:getSpineId()]:changeInnerDir(uv0.getSign(slot2.localScale.x * slot0.shipModels[slot1:getSpineId()].tf.localScale.x < 0))
...
function slot0.changeConflictGridColor(slot0, slot1, slot2, slot3)
	for slot8, slot9 in pairs(slot4) do
		uv0.changeGridColor(slot9, BackYardConst.BACKYARD_GREEN)
	end

	if slot3 then
		for slot8, slot9 in ipairs(slot4) do
			uv0.changeGridColor(slot9, BackYardConst.BACKYARD_BLUE)
		end
	end

	for slot8, slot9 in pairs(slot1) do
		uv0.changeGridColor(slot4[slot9], BackYardConst.BACKYARD_RED)
	end
end
...
function slot0.loadBoatModal(slot0, slot1, slot2)
	PoolMgr.GetInstance():GetSpineChar(slot1:getPrefab(), true, function (slot0)
		if not IsNil(uv0._tf) then
			...
			slot2 = BackYardShipModel.New(slot1, uv1)
			uv0.shipModels[uv1.id] = slot2

			slot2:onLoadSlotModel(uv0)
