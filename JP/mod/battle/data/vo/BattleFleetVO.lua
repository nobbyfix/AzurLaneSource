...
function slot7.FleetWarcry(slot0)
	...
	slot1:DispatchChat(uv0.Battle.BattleDataFunction.GetWords(slot1:GetSkinID(), slot5, slot1:GetDefaultSkinID()), 2.5, "battle")
end

function slot7.SubWarcry(slot0)
	slot0:GetSubList()[1].DispatchVoice(slot1, slot2)
	slot0.GetSubList()[1]:DispatchChat(uv0.Battle.BattleDataFunction.GetWords(slot0.GetSubList()[1].GetSkinID(slot1), slot2, slot0.GetSubList()[1].GetDefaultSkinID(slot1)), 2.5, "battle")
end
...
function slot7.CastChargeWeapon(slot0)
	...
	if slot0._chargeWeaponVO:GetCurrentWeapon() ~= nil and slot1:GetCurrentState() == slot1.STATE_READY then
		slot1:Charge()
		slot0:DispatchEvent(uv0.Event.New(uv0.Battle.BattleUnitEvent.POINT_HIT_CHARGE, slot2))
	end
end
...
function slot7.CancelChargeWeapon(slot0)
	if slot0._chargeWeaponVO:GetCurrentWeapon() ~= nil and slot1:GetCurrentState() == slot1.STATE_PRECAST then
		slot0:DispatchEvent(uv0.Event.New(uv0.Battle.BattleUnitEvent.POINT_HIT_CANCEL, slot2))
		slot1:CancelCharge()
	end
end
...
function slot7.UnleashChrageWeapon(slot0)
	...
	if slot0._chargeWeaponVO:GetCurrentWeapon() ~= nil and slot1:GetCurrentState() == slot1.STATE_PRECAST then
		...
		slot0:DispatchEvent(uv1.Event.New(uv1.Battle.BattleUnitEvent.POINT_HIT_CANCEL, slot2))
	end
end
...
function slot7.UnleashAllInStrike(slot0)
	...
	if slot0._airAssistVO:GetCurrentWeapon() and slot1:GetCurrentState() == slot1.STATE_READY then
		if slot0._IFF == uv0.FRIENDLY_CODE then
			slot0._airAssistVO:PlayCutIn(slot1:GetHost(), 1)
		end
...
function slot7.CoupleEncourage(slot0)
	slot1 = {
		[uv0.GetPlayerShipModelFromID(slot13).group_type] = true
	}
	slot6 = {
		[slot12] = slot22
	}
...
function slot7.onUnitUpdateHP(slot0, slot1)
	for slot7, slot8 in ipairs(slot0._unitList) do
		slot8:TriggerBuff(uv0.BuffEffectType.ON_FRIENDLY_HP_RATIO_UPDATE, {
			unit = slot1.Dispatcher,
			dHP = slot1.Data.dHP
		})

		if slot8 ~= slot1.Dispatcher then
...
function slot7.AddSubMarine(slot0, slot1)
	...
	uv0.Battle.BattleUnitPhaseSwitcher.New(slot1).SetTemplateData(slot3, uv1.GeneratePlayerSubmarinPhase(slot0._subAttackBaseLine, slot0._subRetreatBaseLine, slot1:GetAttrByName("raidDist"), function ()
...
function slot7.ChangeSubmarineState(slot0, slot1)
	for slot5, slot6 in ipairs(slot0._manualSubList) do
		slot6:ChangeOxygenState(slot1)
	end

	for slot5, slot6 in ipairs(slot0._submarineVOList) do
		slot6:ResetCurrent()
	end

	slot0:DispatchEvent(uv0.Event.New(uv1.MANUAL_SUBMARINE_SHIFT, {
		state = slot1
	}))
end

function slot7.SubmarinBoost(slot0)
	for slot4, slot5 in ipairs(slot0._manualSubList) do
		slot5:Boost(Vector3.right, uv0.SR_CONFIG.BOOST_SPEED, uv0.SR_CONFIG.BOOST_DECAY, uv0.SR_CONFIG.BOOST_DURATION, uv0.SR_CONFIG.BOOST_DECAY_STAMP)
	end
