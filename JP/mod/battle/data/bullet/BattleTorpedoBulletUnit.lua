...
function slot2.calcSpeed(slot0)
	slot0._speed = Vector3(uv1.ConvertBulletSpeed(math.max(0, slot0._velocity + uv0.Battle.BattleAttr.GetCurrent(slot0, "torpedoSpeedExtra")) * (1 + uv0.Battle.BattleAttr.GetCurrent(slot0, "bulletSpeedRatio"))) * math.cos(slot4), 0, uv1.ConvertBulletSpeed(math.max(0, slot0._velocity + uv0.Battle.BattleAttr.GetCurrent(slot0, "torpedoSpeedExtra")) * (1 + uv0.Battle.BattleAttr.GetCurrent(slot0, "bulletSpeedRatio"))) * math.sin(math.deg2Rad * slot0._yAngle))
end
...
function slot2.InitCldComponent(slot0)
	...
	if slot0._tempData.extra_param.diveFilter and #slot1 == 0 then
		slot0:GetCldData().Surface = uv1.Battle.BattleConst.OXY_STATE.DIVE
	end
end
