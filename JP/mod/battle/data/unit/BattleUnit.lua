...
function slot9.UpdateWeapon(slot0, slot1)
	...
	if not slot0._antiSubVigilanceState or slot0._antiSubVigilanceState:IsWeaponUseable() then
		if (slot0._weaponRightBound == nil or slot0._move:GetPos().x < slot3) and (slot0._weaponLowerBound == nil or slot0._weaponLowerBound < slot0._move.GetPos().z) then
...
function slot9.UpdateHP(slot0, slot1, slot2, slot3, slot4)
	...
	if not slot2.isHeal then
		slot0:TriggerBuff(uv0.BuffEffectType.ON_TAKE_DAMAGE, {
			damage = -slot1,
		...
		slot0:TriggerBuff(uv0.BuffEffectType.ON_TAKE_HEALING, {
			damage = slot1,
			isHeal = slot7
		})
	...
	if slot3 and not slot3:EqualZero() then
		slot3:Clone().x = Mathf.Clamp(slot3.Clone().x, slot0:GetPosition().x - slot0:GetBoxSize().x, slot0.GetPosition().x + slot0.GetBoxSize().x)
	end
...
function slot9.configWeaponQueueParallel(slot0)
	slot0._weaponQueue:ConfigParallel(slot0:GetManualWeaponParallel()[uv0.ManualWeaponIndex.CALIBRATION], slot0.GetManualWeaponParallel()[uv0.ManualWeaponIndex.TORPEDO])
end
...
function slot9.SetAI(slot0, slot1)
	...
	slot0._autoPilotAI = uv1.Battle.AutoPilot.New(slot0, slot2)
end
...
function slot9.GetOxygenProgress(slot0)
	...
	elseif slot2 == uv0.Battle.FreeDiveOxyState.__name or slot2 == uv0.Battle.FreeFloatOxyState.__name then
