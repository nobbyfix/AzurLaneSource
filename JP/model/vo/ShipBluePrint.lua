...
function slot0.getTaskStateById(slot0, slot1)
	...
	elseif getProxy(TaskProxy):getTaskForBluePrint(slot1) and slot5:isReceive() then
...
function slot0.getEquipProficiencyList(slot0, slot1)
	...
	for slot7, slot8 in pairs(slot2) do
		Clone(slot1:getConfig("equipment_proficiency"))[slot7] = Clone(slot1.getConfig("equipment_proficiency"))[slot7] + slot8
	end
...
function slot0.isFinishPrevTask(slot0, slot1)
	for slot7, slot8 in ipairs(slot2) do
		if not getProxy(TaskProxy):getTaskForBluePrint(slot8) or (slot1 or not nil:isFinish()) and not nil:isReceive() then
			return false
		end
	end
