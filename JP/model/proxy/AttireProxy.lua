...
function slot0.register(slot0)
	...
	slot0:on(11003, function (slot0)
		for slot4, slot5 in ipairs(slot0.icon_frame_list) do
			uv0.data.iconFrames[slot5.id].updateData(slot6, slot5)
			uv0:updateAttireFrame(slot6)
			uv0:addExpiredTimer(uv0.data.iconFrames[slot5.id])
		end
		...
		for slot4, slot5 in slot1(slot2) do
			uv0.data.chatFrames[slot5.id].updateData(slot6, slot5)
			uv0:updateAttireFrame(slot6)
			uv0:addExpiredTimer(uv0.data.chatFrames[slot5.id])
		end
	end)
...
function slot0.addExpiredTimer(slot0, slot1)
	...
	function slot2()
		if getProxy(PlayerProxy).getData(slot0).getAttireByType(slot1, uv0:getType()) == uv0.id then
