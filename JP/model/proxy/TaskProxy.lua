		...
		for slot4, slot5 in ipairs(slot0.info) do
			if uv0.data[slot5.id] ~= nil then
				...
				if slot6:getConfig("type") == 10 and slot6:isFinish() then
					uv0:sendNotification(GAME.SUBMIT_TASK, slot6.id)
				end
			end
		end
	end)
	slot0:on(20003, function (slot0)
		if uv0.onAchieved then
			uv0:addTmpTask(slot0)
...
function slot0.isFinishPrevTasks(slot0, slot1)
	if Task.New({
		...
		return _.all(slot3, function (slot0)
			return (uv0:getTaskById(slot0) or uv0:getFinishTaskById(slot0)) and uv0.getTaskById(slot0) or uv0.getFinishTaskById(slot0):isReceive()
		end)
	end
