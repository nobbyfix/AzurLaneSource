...
function slot0.register(slot0)
	...
	slot0:on(22001, function (slot0)
		...
		ResourceField.New(ResourceField.TYPE_OIL):SetLevel(slot0.oil_well_level > 0 and slot0.oil_well_level or 1)
		...
		slot4 = {
			[Student.New(slot9).id] = Student.New(slot9)
		}
...
function slot0.isResourceFieldUpgradeConditionSatisfy(slot0)
	if slot0:GetOilVO():CanUpgrade(getProxy(PlayerProxy).getData(slot1).level, getProxy(PlayerProxy).getData(slot1).gold) or slot0:GetGoldVO():CanUpgrade(slot2.level, slot2.gold) then
		return true
	end
		...
		if slot2 > 0 then
			while slot2 > 0 and slot11 > 0 do
				...
				for slot24, slot25 in ipairs(slot17) do
					if slot25.skin_type == Ship.SKIN_TYPE_DEFAULT or table.contains(slot9, slot25.id) or slot26 == Ship.SKIN_TYPE_REMAKE and slot6[slot10[math.random(#slot10)]].trans or slot26 == Ship.SKIN_TYPE_PROPOSE and slot6[slot10[math.random(#slot10)]].married == 1 then
					end
