...
function slot0.didEnter(slot0)
	...
	onButton(slot0, slot0.btnBack, function ()
		if isAiriJP() and uv0.agreeActive then
	...
	onToggle(slot0, slot0.stopRemind, function (slot0)
		uv0:emit(uv1.SET_STOP_REMIND, slot0)
	end)
	...
	if isAiriJP() then
		...
		onButton(slot0, slot0.userAgreeBtn1, function ()
			uv0:activeUserAgree(require("ShareCfg.UserAgreement").content, true)
		end, SFX_PANEL)
		onButton(slot0, slot0.userAgreeBtn2, function ()
			uv0:activeUserAgree(require("ShareCfg.UserAgreement2").content, true)
		end, SFX_PANEL)
...
function slot0.updateNotices(slot0, slot1)
	...
	onNextTick(function ()
		if uv0.trContent.childCount > 0 then
			triggerToggle(uv0.trContent:GetChild(0):Find("title"), true)
		end
	end)
end
...
function slot0.updateChild(slot0, slot1, slot2)
	...
	onToggle(slot0, slot3, function (slot0)
		...
		uv1:SetActive(not uv0.isRead)
		uv2:SetActive(slot0)
		Canvas.ForceUpdateCanvases()

		uv3.preferredHeight = slot0 and 52 + uv2.transform.rect.height or 52
	end, SFX_PANEL)
end
	...
	for slot5 = 0, slot0.bannerDots.childCount - 1, 1 do
		...
		onButton(slot0, slot0.bannerContent.GetChild(slot5), function ()
			...
			elseif uv0.type == 2 then
				uv1:emit(uv2.GO_SCENE, uv0.param)
			elseif uv0.type == 3 then
				uv1:emit(uv2.OPEN_ACTIVITY_PANEL, uv0.param)
			end
		end, SFX_PANEL)
	end
end
