...
function slot0.handleNotification(slot0, slot1)
	if slot1:getName() == GAME.COMPOSITE_EQUIPMENT_DONE then
		slot4 = slot1:getBody().equipment

		slot0.viewComponent:filter(slot0.contextData.index or 1)
		pg.TipsMgr:GetInstance():ShowTips(i18n("equipment_newEquipLayer_getNewEquip", slot0.viewComponent.filter .. " X" .. slot1.getBody().count))
