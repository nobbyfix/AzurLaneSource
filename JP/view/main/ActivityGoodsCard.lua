...
function slot0.update(slot0, slot1, slot2)
	...
	if string.match(slot1.getConfig("commodity_type") == DROP_TYPE_SKIN and (pg.ship_skin_template[slot6].name or "??") or slot7.cfg.name or "??", "(%d+)") then
	...
	elseif slot5 == DROP_TYPE_SKIN and not slot3 then
		slot0.limitCountTF.text = "0/" .. slot1:getConfig("num_limit")
	else
		slot0.limitCountTF.text = slot11 - slot1.buyCount .. "/" .. slot11
	end

	slot0.limitCountTF.color = Color.New(uv0.Color[slot2] or uv0.DefaultColor[1], uv0.Color[slot2] or uv0.DefaultColor[2], uv0.Color[slot2] or uv0.DefaultColor[3], 1)
	slot0.limitCountLabelTF.color = Color.New(uv0.Color[slot2] or uv0.DefaultColor[1], uv0.Color[slot2] or uv0.DefaultColor[2], uv0.Color[slot2] or uv0.DefaultColor[3], 1)
end
