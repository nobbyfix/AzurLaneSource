...
function slot0.setItem(slot0, slot1)
	...
	if slot0.contextData.mine and slot0.itemVO:getTempCfgTable() then
		...
		if slot0.itemVO:getConfig("type") == uv0 or uv1 == slot4 then
			setActive(slot0.useBtn, true)
			setText(slot0.useBtn:Find("text"), 1)
...
function slot0.setShipId(slot0, slot1)
	...
	for slot13 = 0, slot0.stars.childCount - 1, 1 do
		...
		SetActive(slot14:Find("star_tpl"), slot8 - slot7 <= slot13)
		SetActive(slot14:Find("star_empty_tpl"), slot13 < slot8 - slot7)
	end
...
function slot0.didEnter(slot0)
	...
	onButton(slot0, slot0.composeBtn, function ()
		SetActive(uv0.composePanel, true)
		SetActive(uv0.window, false)
		SetActive(uv0.top, false)
		uv0:setComposeCount(1)
	end, SFX_CONFIRM)
	onButton(slot0, slot0.composeLeftButton, function ()
		uv0:setComposeCount(uv0.composeCount - 1)
	end)
	onButton(slot0, slot0.composeRightButton, function ()
		uv0:setComposeCount(uv0.composeCount + 1)
	end)
	onButton(slot0, slot0.composeMaxButton, function ()
		uv0:setComposeCount(uv0.composeMax)
	end, SFX_PANEL)
	onButton(slot0, slot0.composeCancel, function ()
		SetActive(uv0.composePanel, false)
		SetActive(uv0.window, true)
		SetActive(uv0.top, true)
	end, SFX_CANCEL)
	onButton(slot0, slot0.composeConfirm, function ()
		uv0:emit(ItemInfoMediator.COMPOSE_ITEM, uv0.itemVO.id, uv0.composeCount)

		if uv0.itemVO.count - uv0.composeCount * uv0.itemVO:getTempCfgTable().compose_number < uv0.itemVO.getTempCfgTable().compose_number then
			triggerButton(uv0.composeCancel)
		else
			uv0:setComposeCount(1)
		end
	end, SFX_CONFIRM)
