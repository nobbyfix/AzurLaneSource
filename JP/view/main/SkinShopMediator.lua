...
function slot0.handleNotification(slot0, slot1)
	if slot1:getName() == GAME.SHOPPING_DONE then
		...
		slot0.viewComponent:onBuyDone(slot1:getBody().id)
		slot0.viewComponent:updateShipRect()

		if pg.shop_template[slot1.getBody().id] and slot5.genre == ShopArgs.SkinShop or slot5.genre == ShopArgs.SkinShopTimeLimit then
