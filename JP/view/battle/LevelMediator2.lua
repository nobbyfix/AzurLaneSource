...
function slot0.register(slot0)
	...
	slot0:bind(uv0.ON_SELECT_COMMANDER, function (slot0, slot1, slot2, slot3)
		...
		uv0.contextData.selectedChapterVO = getProxy(ChapterProxy).getChapterById(slot4, slot3)
	end)
	slot0:bind(uv0.ON_SELECT_ELITE_COMMANDER, function (slot0, slot1, slot2, slot3)
		...
		uv0.contextData.editEliteChapter = slot5
		...
		uv0:sendNotification(GAME.GO_SCENE, SCENE.COMMANDROOM, {
			...
			onSelected = function (slot0, slot1)
				if uv0[uv1] ~= slot0[1] then
					slot4 = getProxy(CommanderProxy).getCommanderById(slot3, slot2)
					slot6 = pairs
					slot7 = uv2:getFleetCommander(uv3, uv4) or {}

					for slot9, slot10 in slot6(slot7) do
						if slot3:getCommanderById(slot10).groupId == slot4.groupId and slot9 ~= uv1 then
				...
				for slot7, slot8 in pairs(uv5) do
					slot8:updateCommander(uv4, uv1, slot2)
					uv2:updateChapter(slot8)
				end

				uv6:updateCommander(uv4, uv1, slot2)
				uv2:updateChapter(uv6)
				slot1()
			end,
			onQuit = function (slot0)
				for slot4, slot5 in pairs(uv0) do
					slot5:updateCommander(uv1, uv2, nil)
					uv3:updateChapter(slot5)
				end

				uv4:updateCommander(uv1, uv2, nil)
				uv3:updateChapter(uv4)
				slot0()
			end
		})
	end)
	...
	slot0:bind(uv0.ON_OP, function (slot0, slot1)
		if uv0.contextData.chapterVO.getDataType(slot2) == ChapterConst.TypeSham then
	...
	slot0:bind(uv0.ON_SWITCH_NORMAL_MAP, function (slot0)
		...
		if Map.lastMap and uv0.viewComponent.maps[Map.lastMap] and not uv0.viewComponent.maps[Map.lastMap]:isEscort() then
			slot1 = Map.lastMap
		else
	...
	slot0:bind(uv0.ON_ELITE_CLEAR, function (slot0, slot1)
		...
		uv0.viewComponent.levelFleetView:setOnHard(slot1.chapterVO)
	end)
	...
	slot0:bind(uv0.ON_ACTIVITY_MAP, function ()
		slot5, slot2 = getProxy(ChapterProxy).getLastMapForActivity(slot0)
	...
	slot0:bind(uv0.ON_OPEN_GUILD_PRE_COMABT, function ()
		uv0:addSubLayers(Context.New({
			...
			data = {
				guildChapter = getProxy(ChapterProxy).getGuildChapter(slot0)
			},
			onRemoved = function ()
				uv0.viewComponent:onSubLayerClose()
			end
	...
	slot0:bind(uv0.ON_FETCH_ESCORT, function (slot0)
		uv0:sendNotification(GAME.ESCORT_FETCH)
	end)
	...
	elseif slot2 == ChapterProxy.GUILD_CHAPTER_UPDATED then
		...
		if slot2 == GAME.CHAPTER_OP_DONE or slot2 == GAME.SHAM_OP_DONE or slot2 == GAME.GUILD_OP_DONE then
			...
			slot4 = coroutine.create(function ()
				...
				if uv0.type == ChapterConst.OpRetreat and uv1.contextData.chapterVO:existOni() and uv1.contextData.chapterVO:checkOniState() then
					uv1.viewComponent:displaySpResult(slot2, uv2)
					coroutine.yield()
				end
				...
				if uv0.items and #slot2 > 0 then
					if slot0 == ChapterConst.OpRetreat and uv1.contextData.map:isEscort() then
						...
						uv1.viewComponent:emit(BaseUI.ON_ACHIEVE, slot2, function ()
				...
				slot3 = uv1.contextData.chapterVO
				...
				elseif slot0 == ChapterConst.OpRetreat then
					...
					if uv3 == GAME.SHAM_OP_DONE and not slot4:getContextByMediator(ShamPreCombatMediator) then
				...
				elseif slot0 == ChapterConst.OpMove then
					seriesAsync({
						...
						function (slot0)
							if uv0.aiActs then
								uv1:playAIActions(uv0.aiActs, slot0)
							else
								slot0()
							end
						end
				...
				elseif slot0 == ChapterConst.OpBox then
					if pg.box_data_template[slot3:getChapterCell(slot3.fleet.line.row, slot3.fleet.line.column).attachmentId].type == ChapterConst.BoxAirStrike then
						...
						if uv0.aiActs and #uv0.aiActs > 0 then
							...
							coroutine.yield()
							uv1.viewComponent:easeAvoid(uv1.viewComponent.grid.cellFleets[slot3.findex].tf.position, uv2)
							coroutine.yield()
						end
				...
				elseif slot0 == ChapterConst.OpEnemyRound then
					uv1:playAIActions(uv0.aiActs, function ()
						uv0.contextData.chapterVO.roundIndex = uv0.contextData.chapterVO.roundIndex + 1
				...
				elseif slot0 == ChapterConst.OpSubState then
					uv1:saveSubState(slot3.subAutoAttack)
					...
					if uv0.arg1 == ChapterConst.StrategyExchange then
						for slot9, slot10 in ipairs(slot5) do
							if slot10:GetType() == FleetSkill.TypeStrategy and slot10:GetArgs()[1] == ChapterConst.StrategyExchange then
								uv1.viewComponent:doPlayCommander(slot3.fleet:findCommanderBySkillId(slot10.id))
		...
		elseif slot2 == GAME.SUB_CHAPTER_REFRESH_DONE then
			...
			slot5 = coroutine.create(function ()
				...
				uv0.viewComponent:doSignalSearch(uv1)
				coroutine.yield()
				...
				if uv0.contextData.map.id ~= uv2:getConfig("map") then
					pg.MsgboxMgr.GetInstance():ShowMsgBox({
...
function slot0.getDockCallbackFuncs(slot0, slot1, slot2, slot3, slot4)
	...
	return function (slot0, slot1)
		...
		if uv2 then
			table.remove(uv0.getEliteFleetList()[uv1], table.indexof(uv0.getEliteFleetList()[uv1], uv2.id))
		end
...
function slot0.playAIActions(slot0, slot1, slot2)
	...
	slot4 = coroutine.create(function ()
		...
		for slot4, slot5 in ipairs(uv1) do
			...
			uv0:playAIAction(slot5, function ()
				slot0, slot1 = uv0:applyTo(uv1, false)
		...
		if #slot0 > 0 then
			...
			for slot5 = 1, #slot0, 1 do
				uv0.viewComponent.grid:showFleetPoisonDamage(slot0[slot5], function ()
					uv0 = uv0 + 1

					if uv0 >= #uv1 then
						uv2()
						uv3.viewComponent:unfrozen()
					end
				end)
			end
...
function slot0.playAIAction(slot0, slot1, slot2)
	...
	if isa(slot1, ChapterAIAction) then
		if slot0.contextData.chapterVO:getChampionIndex(slot1.line.row, slot1.line.column) then
		...
		if slot3:getChapterCell(slot1.line.row, slot1.line.column) and slot5.attachment == ChapterConst.AttachLandbase and pg.land_based_template[slot5.attachmentId].type == ChapterConst.LBCoastalGun then
			if slot0.viewComponent:doPlayAnim("coastalgun", function (slot0)
				setActive(slot0, false)
				uv2.viewComponent:easeMoveDown(uv2.viewComponent.grid.cellFleets[uv0:getFleetIndex(FleetType.Normal, uv1.stgTarget.row, uv1.stgTarget.column)].tf.position, uv3)
			end) or false then
			end
		end
		...
		elseif slot1.target then
			...
			if _.detect(slot1.cellUpdates, function (slot0)
				...
				if pg.land_based_template[slot6.attachmentId].type == ChapterConst.LBCoastalGun then
					if table.contains(ShipType.BundleList[ShipType.BundleAircraftCarrier], slot3:getMapShip(slot3.fleets[slot4]):getShipType()) then
			...
			slot0.viewComponent:doPlayCommander(slot5:findCommanderBySkillId(slot8), function ()
				if uv0:GetType() == FleetSkill.TypeAttack then
					if uv0:GetArgs()[1] == "airfight" then
						uv1.viewComponent:doPlayAirStrike(ChapterConst.SubjectPlayer, true, function ()
							uv0.viewComponent:strikeEnemy(uv1.target, uv2, uv3)
						end)
					elseif slot0[1] == "torpedo" then
						uv1.viewComponent:doPlayStrikeAnim(uv5:getTorpedoShip(uv6), "SubTorpedoUI", function ()
							uv0.viewComponent:strikeEnemy(uv1.target, uv2, uv3)
						end)
