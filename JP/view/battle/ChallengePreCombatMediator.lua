...
function slot0.register(slot0)
	...
	slot0:bind(uv0.ON_UPDATE_FLEET, function (slot0, slot1)
		uv0:getCurrentChallengeInfo().fleet = slot1

		uv0:updateChallenge(uv0.getCurrentChallengeInfo())
	end)
	slot0:bind(uv0.ON_START, function (slot0)
		uv1:sendNotification(GAME.BEGIN_STAGE, {
			system = SYSTEM_CHALLENGE,
			stageId = uv0:getChallengeStageID()
		})
	end)
	slot0:bind(uv0.ON_OP, function (slot0, slot1)
		uv0:sendNotification(GAME.CHALLENGE_STRATEGY, slot1)
	end)
