...
slot0.OPEN_INTENSIFY = "ShipInfoMediator:OPEN_INTENSIFY"
slot0.CLOSE_INTENSIFY = "ShipInfoMediator:CLOSE_INTENSIFY"
slot0.OPEN_EVALUATION = "ShipInfoMediator:OPEN_EVALUATION"
slot0.CLOSE_UPGRADE = "ShipInfoMediator:CLOSE_UPGRADE"
slot0.CHANGE_SKIN = "ShipInfoMediator:CHANGE_SKIN"
slot0.BUY_ITEM = "ShipInfoMediator:BUY_ITEM"
slot0.UNEQUIP_FROM_SHIP_ALL = "ShipInfoMediator:UNEQUIP_FROM_SHIP_ALL"
slot0.NEXTSHIP = "ShipInfoMediator:NEXTSHIP"
slot0.OPEN_ACTIVITY = "ShipInfoMediator:OPEN_ACTIVITY"
slot0.PROPOSE = "ShipInfoMediator:PROPOSE"
...
function slot0.register(slot0)
	...
	slot0:bind(uv0.OPEN_SHIPPROFILE, function (slot0, slot1)
	...
	slot0:bind(uv0.ON_SELECT_EQUIPMENT, function (slot0, slot1)
		for slot10, slot11 in ipairs(slot3) do
			if not slot5:isForbiddenAtPos(slot11, slot1) then
				table.insert(getProxy(BayProxy):getEquipsInShips(slot5, slot1), slot11)
			end
		end
	...
	slot0:bind(uv0.MARK_MAXLEVELHELP_FLAG, function (slot0)
		getProxy(SettingsProxy):setMaxLevelHelp(true)
		uv0.viewComponent:setMaxLevelHelpFlag(true)
	end)
...
function slot0.getEquipmentSkins(slot0, slot1, slot2)
	...
	for slot11, slot12 in ipairs(_.map(slot5, function (slot0)
		return {
			isSkin = true,
			count = 1,
			id = slot0.id,
		...
		table.insert(_.map(slot4, function (slot0)
			return {
				isSkin = true,
				id = slot0.id,
				count = slot0.count
...
function slot0.nextPage(slot0, slot1, slot2)
	...
	for slot10 = slot3, slot4, slot5 do
		if slot0.contextData.shipVOs[slot10] and slot0.bayProxy:getShipById(slot11.id) then
			nil.bindingData = slot11.bindingData
			slot0.contextData.index = slot10
			slot0.contextData.shipId = nil.id
	...
	if slot6 then
		...
		if slot0.viewComponent.page == ShipInfoScene.PAGE.UPGRADE then
			slot0:closeUpgrade()
		elseif slot7 == ShipInfoScene.PAGE.INTENSIFY and not slot0.intensifyContext then
			slot0:closeIntensify()
		elseif slot7 == ShipInfoScene.PAGE.EQUIPMENT and slot0.contextData.isInEquipmentSkinPage and (not slot6:hasEquipEquipmentSkin() or not slot6:canModifyShip()) then
...
function slot0.handleNotification(slot0, slot1)
	...
	elseif slot2 == ShipSkinProxy.SHIP_SKINS_UPDATE then
		...
		slot0.viewComponent:updateFashion()
